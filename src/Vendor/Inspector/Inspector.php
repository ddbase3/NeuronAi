...
namespace NeuronAi\Vendor\Inspector;

use NeuronAi\Vendor\Inspector\Exceptions\InspectorException;
use NeuronAi\Vendor\Inspector\Models\Model;
use NeuronAi\Vendor\Inspector\Transports\AsyncTransport;
use NeuronAi\Vendor\Inspector\Transports\TransportInterface;
use NeuronAi\Vendor\Inspector\Models\Error;
use NeuronAi\Vendor\Inspector\Models\Segment;
use NeuronAi\Vendor\Inspector\Models\Transaction;
use NeuronAi\Vendor\Inspector\Transports\CurlTransport;
use Exception;
use Throwable;
...
class Inspector extends Scope
{
    ...
    {
        $this->configuration->setEnabled(\true);
        return $this;
    }
    /**
     * Stop recording.
     ...
     * @throws Exception
     */
    public function reportException(Throwable $exception, bool $handled = \true): Error
    {
        if (!$this->hasTransaction()) {
        ...
        $segment->addContext('Error', ['message' => $error->message, 'class' => $error->class, 'file' => $error->file, 'line' => $error->line]);
        $segment->end();
        return $error;
    }
