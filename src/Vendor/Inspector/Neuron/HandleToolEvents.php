...
trait HandleToolEvents
{
    ...
    public function toolCalling(object $source, string $event, ToolCalling $data, ?string $branchId = null): void
    {
        ...
        $this->toolCalls[$key] = $scope->startSegment(self::SEGMENT_TYPE . '.tool', "tool_call( {$data->tool->getName()} )")->setColor(self::STANDARD_COLOR);
    }
    public function toolCalled(object $source, string $event, ToolCalled $data, ?string $branchId = null): void
    {
        ...
        $segment = $this->toolCalls[$key]->end()->addContext('Properties', $data->tool->getProperties())->addContext('Inputs', $data->tool->getInputs());
        try {
