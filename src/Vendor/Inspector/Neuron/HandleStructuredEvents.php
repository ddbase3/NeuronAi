...
trait HandleStructuredEvents
{
    ...
    protected function schemaGeneration(object $source, string $event, SchemaGeneration $data, ?string $branchId = null): void
    {
        ...
        $key = $branchId . ':schema';
        $this->structuredSegments[$key] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.structured-output', "schema_generate( " . $this->getBaseClassName($data->class) . " )")->setColor(self::STANDARD_COLOR);
    }
    ...
    protected function extracting(object $source, string $event, Extracting $data, ?string $branchId = null): void
    {
        ...
        $this->structuredSegments[$key] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.structured-output', 'extract_output')->setColor(self::STANDARD_COLOR);
    }
    protected function extracted(object $source, string $event, Extracted $data, ?string $branchId = null): void
    {
        ...
        $this->structuredSegments[$key]->end();
        $this->structuredSegments[$key]->addContext('Data', ['response' => $data->message->jsonSerialize(), 'json' => $data->json])->addContext('Schema', $data->schema);
        unset($this->structuredSegments[$key]);
    }
    protected function deserializing(object $source, string $event, Deserializing $data, ?string $branchId = null): void
    {
        ...
        $this->structuredSegments[$key] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.structured-output', "deserialize( " . $this->getBaseClassName($data->class) . " )")->setColor(self::STANDARD_COLOR);
    }
    ...
    protected function validating(object $source, string $event, Validating $data, ?string $branchId = null): void
    {
        ...
        $this->structuredSegments[$key] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.structured-output', "validate( " . $this->getBaseClassName($data->class) . " )")->setColor(self::STANDARD_COLOR);
    }
