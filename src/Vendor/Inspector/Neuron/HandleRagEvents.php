...
trait HandleRagEvents
{
    public function ragRetrieving(object $source, string $event, Retrieving $data, ?string $branchId = null): void
    {
        ...
        $this->segments[$id] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.retrieval', "vector_retrieval( {$questionText} )")->setColor(self::STANDARD_COLOR);
    }
    public function ragRetrieved(object $source, string $event, Retrieved $data, ?string $branchId = null): void
    {
        ...
        if (array_key_exists($id, $this->segments)) {
            ...
            $segment->addContext('Data', ['question' => $questionText, 'documents' => count($data->documents)]);
            $segment->end();
        }
    }
    public function preProcessing(object $source, string $event, PreProcessing $data, ?string $branchId = null): void
    {
        ...
        $segment = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.preprocessing', $data->processor)->setColor(self::STANDARD_COLOR);
    ...
    public function preProcessed(object $source, string $event, PreProcessed $data, ?string $branchId = null): void
    {
        ...
        if (array_key_exists($key, $this->segments)) {
            $this->segments[$key]->end()->addContext('Processed', $data->processed->jsonSerialize());
        }
    }
    public function postProcessing(object $source, string $event, PostProcessing $data, ?string $branchId = null): void
    {
        ...
        $key = $branchId . ':' . $data->processor;
        $segment = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.postprocessing', $data->processor)->setColor(self::STANDARD_COLOR);
        $segment->addContext('Question', $data->question->jsonSerialize())->addContext('Documents', $data->documents);
        $this->segments[$key] = $segment;
    }
    public function postProcessed(object $source, string $event, PostProcessed $data, ?string $branchId = null): void
    {
        $key = $branchId . ':' . $data->processor;
        if (array_key_exists($key, $this->segments)) {
            $this->segments[$key]->end()->addContext('PostProcess', $data->documents);
        }
    }
}
