...
use NeuronAi\Vendor\Inspector\Models\Token;
...
trait HandleInferenceEvents
{
    ...
    public function messageSaving(object $source, string $event, MessageSaving $data, ?string $branchId = null): void
    {
        ...
        $this->segments[$key] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.chathistory', "save_message( {$label} )")->setColor(self::STANDARD_COLOR);
        if ($data->message instanceof AssistantMessage && $data->message->getUsage() instanceof Usage) {
            ...
            $token->setInputTokens($data->message->getUsage()->inputTokens)->setOutputTokens($data->message->getUsage()->outputTokens);
    ...
    public function inferenceStart(object $source, string $event, InferenceStart $data, ?string $branchId = null): void
    {
        ...
        $label = $this->getBaseClassName($data->message::class);
        $this->inferences[$branchId] = $this->resolveScope($branchId)->startSegment(self::SEGMENT_TYPE . '.inference', "inference( {$label} )")->setColor(self::STANDARD_COLOR);
    }
