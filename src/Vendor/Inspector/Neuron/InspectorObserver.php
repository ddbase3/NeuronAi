...
use Exception;
use NeuronAi\Vendor\Inspector\Configuration;
use NeuronAi\Vendor\Inspector\Exceptions\InspectorException;
use NeuronAi\Vendor\Inspector\Inspector;
use NeuronAi\Vendor\Inspector\Models\Segment;
use NeuronAi\Vendor\Inspector\Scope;
...
use NeuronAi\Vendor\NeuronAI\Observability\ObserverInterface;
...
class InspectorObserver implements ObserverInterface
{
     ...
     */
    protected array $methodsMap = ['error' => 'error', 'workflow-start' => 'workflowStart', 'workflow-resume' => 'workflowStart', 'workflow-end' => 'workflowEnd', 'branch-start' => 'branchStart', 'branch-end' => 'branchEnd', 'workflow-node-start' => 'nodeStart', 'workflow-node-end' => 'nodeEnd', 'middleware-after-start' => 'middlewareStart', 'middleware-after-end' => 'middlewareEnd', 'middleware-before-start' => 'middlewareStart', 'middleware-before-end' => 'middlewareEnd', 'message-saving' => 'messageSaving', 'message-saved' => 'messageSaved', 'tools-bootstrapping' => 'toolsBootstrapping', 'tools-bootstrapped' => 'toolsBootstrapped', 'inference-start' => 'inferenceStart', 'inference-stop' => 'inferenceStop', 'tool-calling' => 'toolCalling', 'tool-called' => 'toolCalled', 'schema-generation' => 'schemaGeneration', 'schema-generated' => 'schemaGenerated', 'structured-extracting' => 'extracting', 'structured-extracted' => 'extracted', 'structured-deserializing' => 'deserializing', 'structured-deserialized' => 'deserialized', 'structured-validating' => 'validating', 'structured-validated' => 'validated', 'rag-retrieving' => 'ragRetrieving', 'rag-retrieved' => 'ragRetrieved', 'rag-preprocessing' => 'preProcessing', 'rag-preprocessed' => 'preProcessed', 'rag-postprocessing' => 'postProcessing', 'rag-postprocessed' => 'postProcessed'];
    protected static ?InspectorObserver $instance = null;
    public function __construct(protected Inspector $inspector, protected bool $autoFlush = \false)
    {
    }
    /**
     * @throws InspectorException
     */
    public static function instance(?string $key = null, ?string $transport = null, ?int $maxItems = null, bool $autoFlush = \false, bool $splitMonitoring = \false): InspectorObserver
    {
    ...
    {
        $mapTool = fn(ToolInterface $tool): array => [$tool->getName() => ['description' => $tool->getDescription(), 'properties' => array_map(fn(ToolPropertyInterface $property) => $property->jsonSerialize(), $tool->getProperties())]];
        return ['Agent' => ['provider' => $agent->resolveProvider()::class, 'instructions' => $agent->resolveInstructions()], 'Tools' => array_map(fn(ToolInterface|ToolkitInterface|ProviderToolInterface $tool) => match (\true) {
