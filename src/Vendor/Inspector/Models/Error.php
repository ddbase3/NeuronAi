...
namespace NeuronAi\Vendor\Inspector\Models;

use NeuronAi\Vendor\Inspector\Models\Partials\Host;
use Exception;
use SplFileObject;
use Throwable;
use function count;
...
class Error extends Model
{
    ...
    public function stackTraceToArray(Throwable $throwable): array
    {
        ...
        $inApp = function (string $file): bool {
            return !str_contains($file, 'vendor') && !str_contains($file, 'index.php') && !str_ends_with($file, 'artisan') && !str_ends_with($file, 'tempest') && !str_contains($file, 'web/core');
            // Drupal
        };
        $stack[] = ['file' => $throwable->getFile(), 'line' => $throwable->getLine(), 'in_app' => $inApp($throwable->getFile()), 'code' => $this->getCode($throwable->getFile(), $throwable->getLine(), $inApp($throwable->getFile()) ? 15 : 5)];
        foreach ($throwable->getTrace() as $trace) {
            $stack[] = ['class' => $trace['class'] ?? null, 'function' => $trace['function'], 'args' => $this->stackTraceArgsToArray($trace), 'type' => $trace['type'] ?? 'function', 'file' => $trace['file'] ?? '[internal]', 'line' => $trace['line'] ?? '0', 'code' => isset($trace['file']) ? $this->getCode($trace['file'], $trace['line'] ?? '0', $inApp($trace['file']) ? 15 : 5) : [], 'in_app' => isset($trace['file']) && $inApp($trace['file'])];
            // Reporting limit
            if (++$counter >= 50) {
                break;
            }
    ...
    public function getCode(string $filePath, int $line, int $linesAround = 5): ?array
    {
        try {
            ...
            while ($file->key() <= $to && !$file->eof()) {
                $codeLines[] = ['line' => $file->key() + 1, 'code' => rtrim($file->current())];
                $file->next();
            }
            return $codeLines;
        } catch (Exception) {
