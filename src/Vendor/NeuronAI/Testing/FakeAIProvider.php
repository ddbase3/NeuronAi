...
class FakeAIProvider implements AIProviderInterface
{
    ...
    public function stream(Message ...$messages): Generator
    {
        ...
        $response = $this->nextResponse();
        $this->recorded[] = new RequestRecord(method: 'stream', messages: $messages, systemPrompt: $this->systemPrompt, tools: $this->tools);
    ...
    public function structured(array|Message $messages, string $class, array $response_schema): Message
    {
        ...
        $response = $this->nextResponse();
        $this->recorded[] = new RequestRecord(method: 'structured', messages: $messages, systemPrompt: $this->systemPrompt, tools: $this->tools, structuredClass: $class, structuredSchema: $response_schema);
        return $response;
    }
    ...
    {
        Assert::assertCount($expected, $this->recorded, "Expected {$expected} provider calls, got " . count($this->recorded) . '.');
    }
    /**
    ...
    {
        Assert::assertEmpty($this->recorded, 'Expected no provider calls, but ' . count($this->recorded) . ' were recorded.');
    }
    /**
    ...
    public function assertMethodCallCount(string $method, int $expected): void
    {
        ...
        Assert::assertSame($expected, $count, "Expected {$expected} '{$method}' calls, got {$count}.");
    }
    /**
    ...
    public function assertToolsConfigured(array $toolNames): void
    {
        $matched = \false;
        foreach ($this->recorded as $record) {
            $recordToolNames = array_values(array_map(static fn(ToolInterface|ProviderToolInterface $tool): string => $tool->getName(), $record->tools));
    ...
    protected function nextResponse(): Message
    {
        if ($this->responseQueue === []) {
            throw new ProviderException('FakeAIProvider response queue is empty. Add more responses with addResponses() or pass them to the constructor.');
        }
