...
use NeuronAi\Vendor\NeuronAI\MCP\McpTransportInterface;
...
class FakeMcpTransport implements McpTransportInterface
{
    ...
    {
        return ['queue' => [...$this->sent, ...$this->responseQueue], 'connected' => $this->connected];
    }
    ...
    {
        Assert::assertCount($expected, $this->sent, "Expected {$expected} send() calls, got " . count($this->sent) . '.');
    }
    public function assertReceiveCount(int $expected): void
    {
        Assert::assertCount($expected, $this->received, "Expected {$expected} receive() calls, got " . count($this->received) . '.');
    }
    public function assertNothingSent(): void
    {
        Assert::assertEmpty($this->sent, 'Expected no data to be sent, but ' . count($this->sent) . ' sends were recorded.');
    }
    public function assertNothingReceived(): void
    {
        Assert::assertEmpty($this->received, 'Expected no data to be received, but ' . count($this->received) . ' receives were recorded.');
    }
    /**
    ...
    public function assertMethodSent(string $method, int $expectedCount = 1): void
    {
        ...
        Assert::assertSame($expectedCount, $count, "Expected {$expectedCount} sends with method '{$method}', got {$count}.");
    }
    /**
    ...
    public function assertMethodReceived(string $method, int $expectedCount = 1): void
    {
        ...
        Assert::assertSame($expectedCount, $count, "Expected {$expectedCount} receives with method '{$method}', got {$count}.");
    }
    /**
    ...
    public function assertToolCalled(string $toolName, int $expectedCount = 1): void
    {
        ...
        Assert::assertSame($expectedCount, $count, "Expected {$expectedCount} calls to tool '{$toolName}', got {$count}.");
    }
}
