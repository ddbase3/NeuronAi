...
class FakeEmbeddingsProvider implements EmbeddingsProviderInterface
{
    ...
    {
        Assert::assertCount($expected, $this->recorded, "Expected {$expected} embedding calls, got " . count($this->recorded) . '.');
    }
    public function assertEmbeddedText(string $expected): void
    {
        Assert::assertContains($expected, $this->recorded, "Expected text was never embedded: {$expected}");
    }
    public function assertNothingEmbedded(): void
    {
        Assert::assertEmpty($this->recorded, 'Expected no embedding calls, but ' . count($this->recorded) . ' were recorded.');
    }
    /**
    ...
    protected function deterministicVector(string $text): array
    {
        ...
        return array_map(fn(string $char): float => ord($char) / 255.0, array_slice($bytes, 0, $this->dimensions));
    }
}
