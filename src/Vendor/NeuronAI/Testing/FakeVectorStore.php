...
class FakeVectorStore implements VectorStoreInterface
{
    ...
    {
        $this->documents = array_values(array_filter($this->documents, fn(Document $doc): bool => $doc->sourceType !== $sourceType || $sourceName !== null && $doc->sourceName !== $sourceName));
    ...
    {
        Assert::assertSame($expected, $this->searchCount, "Expected {$expected} similarity searches, got {$this->searchCount}.");
    }
    public function assertDocumentCount(int $expected): void
    {
        Assert::assertCount($expected, $this->documents, "Expected {$expected} documents in store, got " . count($this->documents) . '.');
    }
    ...
    {
        Assert::assertEmpty($this->documents, 'Expected no documents in store, but ' . count($this->documents) . ' were found.');
    }
}
