...
use NeuronAi\Vendor\PHPUnit\Framework\Assert;
use Throwable;
...
class FakeMiddleware implements WorkflowMiddleware
{
    ...
    {
        return array_values(array_filter($this->recorded, static fn(MiddlewareRecord $record): bool => $record->method === 'before'));
    }
    /**
     * @return MiddlewareRecord[]
     */
    ...
    {
        return array_values(array_filter($this->recorded, static fn(MiddlewareRecord $record): bool => $record->method === 'after'));
    }
    // ----------------------------------------------------------------
    // PHPUnit Assertions
    // ----------------------------------------------------------------
    ...
    {
        Assert::assertNotEmpty($this->getBeforeRecords(), 'Expected before() to be called at least once, but it was not called.');
    }
    public function assertBeforeNotCalled(): void
    {
        Assert::assertEmpty($this->getBeforeRecords(), 'Expected before() not to be called, but it was called ' . count($this->getBeforeRecords()) . ' time(s).');
    }
    public function assertAfterCalled(): void
    {
        Assert::assertNotEmpty($this->getAfterRecords(), 'Expected after() to be called at least once, but it was not called.');
    }
    public function assertAfterNotCalled(): void
    {
        Assert::assertEmpty($this->getAfterRecords(), 'Expected after() not to be called, but it was called ' . count($this->getAfterRecords()) . ' time(s).');
    }
    public function assertBeforeCalledTimes(int $expected): void
    {
        $actual = count($this->getBeforeRecords());
        Assert::assertSame($expected, $actual, "Expected before() to be called {$expected} time(s), but it was called {$actual} time(s).");
    }
    public function assertAfterCalledTimes(int $expected): void
    {
        $actual = count($this->getAfterRecords());
        Assert::assertSame($expected, $actual, "Expected after() to be called {$expected} time(s), but it was called {$actual} time(s).");
    }
    /**
    ...
    {
        Assert::assertEmpty($this->recorded, 'Expected middleware not to be called, but it was called ' . count($this->recorded) . ' time(s).');
    }
    public function assertCallCount(int $expected): void
    {
        Assert::assertCount($expected, $this->recorded, "Expected {$expected} total middleware calls, got " . count($this->recorded) . '.');
    }
}
