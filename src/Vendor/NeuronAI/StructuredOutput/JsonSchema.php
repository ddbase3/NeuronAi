...
namespace NeuronAi\Vendor\NeuronAI\StructuredOutput;
...
class JsonSchema
{
    ...
    public function generate(string $class): array
    {
        ...
        return [...$this->generateClassSchema($class), 'additionalProperties' => \false];
    }
    /**
    ...
    protected function generateClassSchema(string $class): array
    {
        ...
        foreach ($properties as $property) {
            ...
            if ($attribute instanceof SchemaProperty && $attribute->required !== null) {
                ...
                $isNullable = $type ? $type->allowsNull() : \true;
    ...
    protected function injectDiscriminator(array $schema, string $discriminatorValue): array
    {
        ...
        if (isset($schema['type']) && $schema['type'] === 'object') {
            ...
            $schema['properties'] = [$this->discriminator => ['type' => 'string', 'enum' => [$discriminatorValue], 'description' => 'This property is mandatory and can only be filled with "' . $discriminatorValue . '". It is used as a discriminator for class type resolution.'], ...$schema['properties'] ?? []];
            // Make __classname__ required
            $schema['required'] = array_unique([$this->discriminator, ...$schema['required'] ?? []]);
        }
        return $schema;
    }
}
