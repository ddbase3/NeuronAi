...
namespace NeuronAi\Vendor\NeuronAI\Workflow\Interrupt;
...
{
    public function __construct(protected InterruptRequest $request, protected NodeInterface $node, protected WorkflowState $state, protected Event $event, protected ?string $branchId = null, protected ?ParallelEvent $parallelEvent = null, protected array $completedBranchResults = [])
    {
    ...
    {
        return ['resumeToken' => $this->getWorkflowId(), 'message' => $this->message, 'request' => serialize($this->request), 'node' => serialize($this->node), 'state' => serialize($this->state), 'currentEvent' => serialize($this->event), 'branchId' => $this->branchId, 'parallelEvent' => $this->parallelEvent instanceof ParallelEvent ? serialize($this->parallelEvent) : null, 'completedBranchResults' => serialize($this->completedBranchResults)];
    }
