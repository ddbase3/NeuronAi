...
abstract class Node implements NodeInterface
{
    ...
    {
        return $this->interruptIf(\true, $request);
    }
    /**
     * @template T of InterruptRequest
     * @param T $request
    ...
    protected function interruptIf(callable|bool $condition, InterruptRequest $request): ?InterruptRequest
    {
        ...
        if ($shouldInterrupt) {
            throw new WorkflowInterrupt($request, $this, $this->state, $this->event);
        }
    ...
    {
        EventBus::emit($event, $this, $data, $this->state->get('__workflowId'), $this->state->get('__branchId'));
    }
}
