...
class EloquentPersistence implements PersistenceInterface, SerializablePersistenceInterface
{
    ...
    public function save(string $workflowId, WorkflowInterrupt $interrupt): void
    {
        ...
        $model->newQuery()->updateOrCreate(['workflow_id' => $workflowId], [
    ...
    public function load(string $workflowId): WorkflowInterrupt
    {
        ...
        $model = new $this->modelClass();
        $record = $model->newQuery()->where('workflow_id', $workflowId)->firstOr(['interrupt'], fn() => throw new WorkflowException("No saved workflow found for ID: {$workflowId}."));
        $interruptData = base64_decode($record->interrupt, \true);
        if ($interruptData === \false) {
            $interruptData = $record->interrupt;
            // This makes sure that previous records still work
        }
    ...
    public function delete(string $workflowId): void
    {
        /** @var Model $model */
        $model = new $this->modelClass();
        $model->newQuery()->where('workflow_id', $workflowId)->delete();
    }
}
