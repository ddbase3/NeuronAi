...
use NeuronAi\Vendor\NeuronAI\StaticConstructor;
use NeuronAi\Vendor\NeuronAI\Workflow\Events\Event;
use NeuronAi\Vendor\NeuronAI\Workflow\Events\StartEvent;
use NeuronAi\Vendor\NeuronAI\Workflow\Executor\WorkflowExecutor;
use NeuronAi\Vendor\NeuronAI\Workflow\Executor\WorkflowExecutorInterface;
use NeuronAi\Vendor\NeuronAI\Workflow\Exporter\ConsoleExporter;
use NeuronAi\Vendor\NeuronAI\Workflow\Exporter\ExporterInterface;
use NeuronAi\Vendor\NeuronAI\Workflow\Interrupt\InterruptRequest;
use NeuronAi\Vendor\NeuronAI\Workflow\Persistence\InMemoryPersistence;
...
class Workflow implements WorkflowInterface
{
     ...
     */
    public function __construct(?PersistenceInterface $persistence = null, ?string $resumeToken = null, protected ?WorkflowState $state = null)
    {
        ...
        foreach ($this->getNodes() as $node) {
            ...
            try {
                ...
                if ($firstParamType instanceof ReflectionNamedType) {
                    ...
                    $eventTypes = array_filter($firstParamType->getTypes(), fn(ReflectionType $type): bool => $type instanceof ReflectionNamedType && is_a($type->getName(), Event::class, \true));
    ...
    {
        if (!isset($this->eventNodeMap[$eventClass])) {
            throw new WorkflowException("No node found that handle event: " . $eventClass);
        }
    ...
    public function setExecutor(WorkflowExecutorInterface $executor): WorkflowInterface
    {
        ...
        if ($persistence instanceof \NeuronAi\Vendor\NeuronAI\Workflow\Persistence\PersistenceInterface) {
    ...
    protected function validateInvokeMethodSignature(NodeInterface $node): void
    {
        try {
            ...
            if (!$firstParamType instanceof ReflectionNamedType || !is_a($firstParamType->getName(), Event::class, \true)) {
            ...
            if (!$secondParamType instanceof ReflectionNamedType || !is_a($secondParamType->getName(), WorkflowState::class, \true)) {
            ...
            if ($returnType instanceof ReflectionNamedType) {
                ...
                if (!is_a($returnType->getName(), Event::class, \true) && !is_a($returnType->getName(), Generator::class, \true)) {
                ...
                foreach ($returnType->getTypes() as $type) {
                    if (!$type instanceof ReflectionNamedType || !is_a($type->getName(), Event::class, \true) && !is_a($type->getName(), Generator::class, \true)) {
