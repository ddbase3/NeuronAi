...
use NeuronAi\Vendor\NeuronAI\Observability\EventBus;
...
class WorkflowExecutor implements WorkflowExecutorInterface
{
    ...
    public function resume(WorkflowInterface $workflow, InterruptRequest $request): Generator
    {
        ...
        try {
            if ($interrupt->isParallelInterrupt()) {
                $gen = $this->executeParallelBranches($workflow, $interrupt->getParallelEvent(), $interrupt, $request);
                yield from $gen;
                $parallelEvent = $gen->getReturn();
                yield from $this->traverseNodes($workflow, $parallelEvent, $workflow->getNodeForEvent($parallelEvent::class));
            } else {
                yield from $this->traverseNodes($workflow, $interrupt->getEvent(), $interrupt->getNode(), $request);
            }
            $this->persistence?->delete($workflowId);
     ...
     * @throws WorkflowException
     */
    protected function traverseNodes(WorkflowInterface $workflow, Event $currentEvent, NodeInterface $currentNode, ?InterruptRequest $resumeRequest = null): Generator
    {
     ...
     * @throws WorkflowException|InspectorException
     */
    protected function executeNode(WorkflowInterface $workflow, Event $currentEvent, NodeInterface $currentNode, ?InterruptRequest $resumeRequest = null, ?WorkflowState $state = null, ?string $branchId = null): Generator
    {
        $state ??= $workflow->resolveState();
        $currentNode->setWorkflowContext($state, $currentEvent, $resumeRequest);
        EventBus::emit('workflow-node-start', $workflow, new WorkflowNodeStart($currentNode::class, $state), $workflow->getWorkflowId(), $branchId);
        try {
            ...
            EventBus::emit('workflow-node-end', $workflow, new WorkflowNodeEnd($currentNode::class, $state), $workflow->getWorkflowId(), $branchId);
        }
        return $nodeResult;
    }
    /**
    ...
    protected function executeParallelBranches(WorkflowInterface $workflow, ParallelEvent $parallelEvent, ?WorkflowInterrupt $interrupt = null, ?InterruptRequest $resumeRequest = null): Generator
    {
        foreach ($parallelEvent->branches as $branchId => $branchEvent) {
            ...
            try {
                $result = $this->executeBranch($workflow, $branchId, $isResuming ? $interrupt->getEvent() : $branchEvent, $isResuming ? $resumeRequest : null, $isResuming ? $interrupt->getNode() : null);
                ...
                throw new WorkflowInterrupt(request: $branchInterrupt->original->getRequest(), node: $branchInterrupt->original->getNode(), state: $workflow->resolveState(), event: $branchInterrupt->original->getEvent(), branchId: $branchInterrupt->branchId, parallelEvent: $parallelEvent, completedBranchResults: $parallelEvent->getAllResults());
            }
        }
     ...
     * @throws WorkflowException|InspectorException
     */
    protected function executeBranch(WorkflowInterface $workflow, string $branchId, Event $branchEvent, ?InterruptRequest $resumeRequest = null, ?NodeInterface $startNode = null): BranchResult
    {
        $streamedEvents = [];
        $branchState = clone $workflow->resolveState();
        $branchState->set('__branchId', $branchId);
        EventBus::emit('branch-start', $workflow, new BranchStart($branchId), $workflow->getWorkflowId(), $branchId);
        try {
            ...
            EventBus::emit('branch-end', $workflow, new BranchEnd($branchId), $workflow->getWorkflowId(), $branchId);
        }
        return new BranchResult(branchId: $branchId, result: $currentEvent->getResult(), streamedEvents: $streamedEvents);
    }
    /**
     * @throws InspectorException
     */
    protected function runBeforeMiddleware(WorkflowInterface $workflow, Event $event, NodeInterface $node, ?WorkflowState $state = null, ?string $branchId = null): void
    {
        $state ??= $workflow->resolveState();
     ...
     * @throws InspectorException
     */
    protected function runAfterMiddleware(WorkflowInterface $workflow, Event $event, NodeInterface $node, ?WorkflowState $state = null, ?string $branchId = null): void
    {
        $state ??= $workflow->resolveState();
        foreach ($workflow->getMiddlewareForNode($node) as $m) {
