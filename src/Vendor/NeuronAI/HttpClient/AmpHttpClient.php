...
use NeuronAi\Vendor\Amp\ByteStream\ReadableResourceStream;
use NeuronAi\Vendor\Amp\Http\Client\Form;
use NeuronAi\Vendor\Amp\Http\Client\HttpClient;
use NeuronAi\Vendor\Amp\Http\Client\HttpClientBuilder;
use NeuronAi\Vendor\Amp\Http\Client\Request;
use NeuronAi\Vendor\Amp\Http\Client\Response;
use NeuronAi\Vendor\Amp\Http\Client\StreamedContent;
...
class AmpHttpClient implements HttpClientInterface
{
     ...
     */
    public function __construct(protected array $customHeaders = [], protected float $timeout = 60.0)
    {
    }
    public function request(HttpRequest $request): HttpResponse
    {
        try {
            ...
            return new HttpResponse(statusCode: $response->getStatus(), body: $response->getBody()->buffer(), headers: $response->getHeaders());
    ...
    protected function executeMultipart(HttpRequest $request): Response
    {
        ...
        $uri = $this->baseUri !== '' && $this->baseUri !== '0' ? trim($this->baseUri, '/') . '/' . trim($request->uri, '/') : $request->uri;
