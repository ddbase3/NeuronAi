...
namespace NeuronAi\Vendor\NeuronAI\HttpClient;
...
use NeuronAi\Vendor\GuzzleHttp\HandlerStack;
...
class GuzzleHttpClient implements HttpClientInterface
{
     ...
     */
    public function __construct(protected array $customHeaders = [], protected float $timeout = 60.0, protected float $connectTimeout = 10.0, protected ?HandlerStack $handler = null, protected array $options = [])
    {
    }
    public function request(HttpRequest $request): HttpResponse
    {
        ...
        try {
            $options = [...$this->options, RequestOptions::HEADERS => [...$this->customHeaders, ...$request->headers], RequestOptions::TIMEOUT => $this->timeout, RequestOptions::CONNECT_TIMEOUT => $this->connectTimeout];
            $response = $this->runRequest($request, $options, $client);
            return new HttpResponse(statusCode: $response->getStatusCode(), body: (string) $response->getBody(), headers: $response->getHeaders());
    ...
    public function stream(HttpRequest $request): StreamInterface
    {
        $client = $this->createClient();
        try {
            $options = [RequestOptions::HEADERS => [...$this->customHeaders, ...$request->headers], RequestOptions::TIMEOUT => $this->timeout, RequestOptions::CONNECT_TIMEOUT => $this->connectTimeout, RequestOptions::STREAM => \true];
            $response = $this->runRequest($request, $options, $client);
    ...
    public function runRequest(HttpRequest $request, array $options, Client $client): ResponseInterface
    {
        ...
        $uri = $this->baseUri !== '' ? trim($this->baseUri, '/') . ($request->uri !== '' ? '/' . trim($request->uri, '/') : '') : $request->uri;
    ...
    protected function handleException(HttpRequest $request, GuzzleException $e): never
    {
        if ($e instanceof RequestException && $e->hasResponse()) {
            ...
            $response = new HttpResponse(statusCode: $psrResponse->getStatusCode(), body: (string) $psrResponse->getBody(), headers: $psrResponse->getHeaders());
            throw new HttpException("HTTP {$response->statusCode} error during {$request->method->value} {$request->uri}: {$response->body}", $request, $response, $e);
        }
        throw new HttpException("Network error during {$request->method->value} {$request->uri}: {$e->getMessage()}", $request, null, $e);
    }
}
