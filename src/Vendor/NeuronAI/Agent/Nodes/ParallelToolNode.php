...
use NeuronAi\Vendor\NeuronAI\Exceptions\ToolRunsExceededException;
...
use NeuronAi\Vendor\NeuronAI\Tools\HasRunKey;
use NeuronAi\Vendor\NeuronAI\Tools\ToolInterface;
use NeuronAi\Vendor\Spatie\Fork\Fork;
use Closure;
use Throwable;
...
class ParallelToolNode extends ToolNode
{
    ...
    protected function executeTools(ToolCallMessage $toolCallMessage, AgentState $state): Generator
    {
        ...
        foreach ($tools as $tool) {
            ...
            $this->emit('tool-calling', new ToolCalling($tool, \true));
            yield new ToolCallChunk($tool);
        }
        // Execute tools concurrently and collect serialized tool states
        $serializedTools = Fork::new()->run(...array_map(fn(ToolInterface $tool): Closure => function () use ($tool): string {
            try {
                ...
                return serialize(['error' => \true, 'exception_class' => $exception::class, 'exception_message' => $exception->getMessage(), 'exception_code' => $exception->getCode(), 'tool_name' => $tool->getName()]);
            }
        }, $tools));
        ...
        foreach ($serializedTools as $index => $serializedTool) {
            ...
            if (is_array($data) && isset($data['error']) && $data['error'] === \true) {
