...
class WriteTodosTool extends Tool
{
    ...
    {
        parent::__construct(name: 'write_todos', description: <<<TODO
...
TODO
    ...
    {
        return [ArrayProperty::make(name: 'todos', description: 'Array of todo items. Each item must have "content" (task description) and "status" (one of: pending, in_progress, completed)', required: \true, items: ObjectProperty::make(name: 'item', description: 'Item in the todo list', required: \true, properties: [ToolProperty::make(name: 'content', type: PropertyType::STRING, description: 'Task description'), ToolProperty::make(name: 'status', type: PropertyType::STRING, description: 'Current status of the task', required: \true, enum: ['pending', 'in_progress', 'completed'])]))];
    }
    /**
    ...
    public function __invoke(array $todos): string
    {
        ...
        foreach ($todos as $index => $todo) {
            ...
            if (!in_array($todo['status'], ['pending', 'in_progress', 'completed'], \true)) {
