...
class ToolSearchTool extends Tool
{
     ...
     */
    public function __construct(protected array $toolPool, protected int $topN = 10)
    {
        parent::__construct(name: 'tool_search', description: 'Search for available tools by name or description. Returns matching tools that will become available for you to use.');
        $this->topN = max(1, $this->topN);
    }
    protected function properties(): array
    {
        return [ToolProperty::make(name: 'query', type: PropertyType::STRING, description: 'Search query to find relevant tools. One word per query for strict matching, multiple words for fuzzy matching.', required: \true)];
    }
    public function __invoke(string $query): string
    {
        ...
        foreach ($matches as $tool) {
            $descriptions[] = sprintf('- %s: %s', $tool->getName(), $tool->getDescription() ?? 'No description');
        }
    ...
    protected function defaultSearch(string $query): array
    {
        ...
        foreach ($this->toolIndex as $entry) {
            ...
            foreach ($queryKeywords as $keyword) {
                ...
                $nameMatched = \false;
                if (in_array($keyword, $toolNameWords, \true)) {
                    $score += 15;
                    $nameMatched = \true;
                ...
                if (!$nameMatched && strlen($keyword) >= 4) {
                    foreach ($toolNameWords as $nameWord) {
                        if (strlen($nameWord) >= 4 && levenshtein($keyword, $nameWord) <= 2) {
                            $score += 5;
                            break;
                            // only match once per keyword
                        }
                    }
                }
                // Check description matches (tiered check)
                $descMatched = \false;
                if (in_array($keyword, $toolDescriptionWords, \true)) {
                    $score += 5;
                    $descMatched = \true;
            ...
            if ($score > 0) {
                $scoredTools[] = ['tool' => $entry['tool'], 'score' => $score];
            }
        }
    ...
    protected function buildToolIndex(): array
    {
        ...
        foreach ($this->toolPool as $tool) {
            ...
            $index[] = ['tool' => $tool, 'nameLower' => strtolower($tool->getName()), 'descLower' => $descLower, 'nameWords' => $this->tokenize($tool->getName()), 'descWords' => $descLower !== '' ? $this->tokenize($descLower) : []];
        }
        return $index;
    }
    /**
    ...
    protected function tokenize(string $text): array
    {
        ...
        return array_values(array_filter(preg_split('/[\s,\.\-_]+/', $spaced), fn(string $word): bool => $word !== ''));
    }
}
