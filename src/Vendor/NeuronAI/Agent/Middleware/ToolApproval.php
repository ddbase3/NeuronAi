...
namespace NeuronAi\Vendor\NeuronAI\Agent\Middleware;

use NeuronAi\Vendor\NeuronAI\Agent\Events\ToolCallEvent;
use NeuronAi\Vendor\NeuronAI\Agent\Nodes\ToolNode;
use NeuronAi\Vendor\NeuronAI\Agent\Tools\ToolRejectionHandler;
use NeuronAi\Vendor\NeuronAI\Tools\ToolInterface;
use NeuronAi\Vendor\NeuronAI\Workflow\Events\Event;
use NeuronAi\Vendor\NeuronAI\Workflow\Interrupt\Action;
use NeuronAi\Vendor\NeuronAI\Workflow\Interrupt\ActionDecision;
use NeuronAi\Vendor\NeuronAI\Workflow\Interrupt\ApprovalRequest;
...
class ToolApproval implements WorkflowMiddleware
{
    ...
    public function before(NodeInterface $node, Event $event, WorkflowState $state): void
    {
        ...
        $count = count($actions);
        $interruptRequest = new ApprovalRequest(message: sprintf('%d tool call%s require%s approval before execution', $count, $count === 1 ? '' : 's', $count === 1 ? 's' : ''), actions: $actions);
    ...
    protected function filterToolsRequiringApproval(array $tools): array
    {
        ...
        return array_filter($tools, $this->toolRequiresApproval(...));
    }
    /**
    ...
    protected function createAction(ToolInterface $tool): Action
    {
        ...
        $inputsDescription = $inputs === [] ? '(no arguments)' : json_encode($inputs, JSON_PRETTY_PRINT);
        return new Action(id: $tool->getCallId() ?? uniqid('tool_'), name: $tool->getName(), description: $inputsDescription, decision: ActionDecision::Pending);
    }
    /**
     ...
     */
    protected function processDecisions(ApprovalRequest $request, ToolCallEvent $event): void
    {
    ...
    protected function handleRejectedTool(ToolInterface $tool, Action $action): void
    {
        ...
        $rejectionMessage = sprintf("TOOL NOT EXECUTED. The user rejected this action. User instruction: %s. Do not attempt this tool again. Follow the user's instruction.", $feedback);
