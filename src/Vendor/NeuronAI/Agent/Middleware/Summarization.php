...
{
    public function __construct(protected AIProviderInterface $provider, protected int $maxTokens = 50000, protected int $messagesToKeep = 5, protected ?string $summaryPrompt = null)
    {
    }
    /**
    ...
    protected function summarizeHistory(AgentState $state, array $messages): void
    {
        ...
        $newMessages = [new UserMessage("## Previous conversation summary:\n\n{$summary}"), ...$recentMessages];
        // Update chat history
    ...
    protected function generateSummary(array $messages): string
    {
        ...
        try {
            ...
            $response = $this->provider->systemPrompt('You are a helpful assistant that creates concise, informative summaries of conversations.')->chat(new UserMessage("{$prompt}\n\n{$conversation}"));
            return $response->getContent();
        } catch (Exception) {
            // If summarization fails, return a basic fallback summary
            return sprintf('Previous conversation contained %d messages covering various topics.', count($messages));
        }
    }
    /**
    ...
    protected function formatMessagesForSummarization(array $messages): string
    {
        ...
        foreach ($messages as $message) {
            ...
            if ($message instanceof ToolCallMessage) {
                ...
                $formatted[] = sprintf('[%s]: Called tools: %s', strtoupper($role), implode(', ', $toolNames));
            } elseif ($message instanceof ToolResultMessage) {
                $formatted[] = sprintf('[%s]: Tool results received', strtoupper($role));
            } else {
                // Regular message - extract text content from blocks
                $contentStr = $message->getContent();
                $formatted[] = sprintf('[%s]: %s', strtoupper($role), $contentStr);
            }
        }
        return implode("\n", $formatted);
    }
