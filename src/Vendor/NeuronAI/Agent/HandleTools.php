...
use NeuronAi\Vendor\NeuronAI\Observability\Events\ToolsBootstrapped;
    ...
    public function bootstrapTools(): array
    {
        ...
        foreach ($this->getTools() as $tool) {
            if ($tool instanceof ToolkitInterface) {
                ...
                if (!in_array($kitGuidelines, [null, '', '0'], \true)) {
                    $kitGuidelines .= PHP_EOL . implode(PHP_EOL . '- ', array_map(fn(ToolInterface $tool): string => "{$tool->getName()}", $innerTools));
        ...
        if ($guidelines !== []) {
            $this->setInstructions($instructions . PHP_EOL . '<TOOLS-GUIDELINES>' . PHP_EOL . implode(PHP_EOL . PHP_EOL, $guidelines) . PHP_EOL . '</TOOLS-GUIDELINES>');
        }
        /*EventBus::emit(
              'tools-bootstrapped',
              $this,
