...
class HistoryTrimmer implements HistoryTrimmerInterface
{
    ...
    protected function normalizeCheckpoints(array $messages, int $trimmedTokens): void
    {
        ...
        foreach ($messages as $message) {
            ...
            if ($usage !== null) {
                ...
                $message->setUsage(new Usage($normalizedInputTokens, $usage->outputTokens));
            }
        }
    ...
    protected function getCheckpoints(array $messages, int $count, string $hash): array
    {
        ...
        foreach ($messages as $index => $message) {
            if ($message instanceof AssistantMessage && ($usage = $message->getUsage()) instanceof Usage) {
                $checkpoints[] = ['index' => $index, 'tokens' => $usage->inputTokens + $usage->outputTokens];
            }
        }
    ...
    protected function findTrimPoint(array $messages, array $checkpoints, int $contextWindow): array
    {
        ...
        foreach ($checkpoints as $checkpoint) {
            if ($checkpoint['tokens'] >= $threshold) {
                return $this->adjustTrimIndex($messages, $checkpoint['index'] + 1, $checkpoint['tokens']);
            }
        }
        // Tail overflow: trim at the last checkpoint
        $lastCheckpoint = end($checkpoints);
        return $this->adjustTrimIndex($messages, $lastCheckpoint['index'] + 1, (int) $lastCheckpoint['tokens']);
    }
    /**
    ...
    {
        return array_reduce($messages, fn(int $carry, Message $message): int => $carry + $this->tokenCounter->count($message), 0);
    }
    /**
        ...
        foreach ($messages as $index => $message) {
            ...
            if ($message instanceof ToolResultMessage) {
                if (!$previousMessage instanceof ToolCallMessage) {
                    throw new ChatHistoryException(sprintf('Invalid message sequence: ToolResultMessage at position %d must follow a ToolCallMessage', $index));
                }
            ...
            if ($message instanceof ToolCallMessage) {
                if ($role !== MessageRole::ASSISTANT->value) {
                    throw new ChatHistoryException(sprintf('Invalid message sequence: ToolCallMessage at position %d must have ASSISTANT role, got %s', $index, $role));
                }
            ...
            if ($role !== $expectedRole) {
                throw new ChatHistoryException(sprintf('Invalid message sequence at position %d: expected role %s, got %s', $index, $expectedRole, $role));
            }
