...
class TokenCounter
{
    public function __construct(protected float $charsPerToken = 4.0, protected float $extraTokensPerMessage = 3.0)
    {
    }
    public function count(Message $message): int
    {
        ...
        $chars = array_reduce($message->getContentBlocks(), fn(float $carry, ContentBlockInterface $block): float => $carry + match ($block::class) {
            TextContent::class, ReasoningContent::class => $this->handleTextBlock($block),
            ImageContent::class => $this->handleImageBlock($block),
            default => 200 * $this->charsPerToken,
        }, $chars);
    ...
    protected function handleToolResult(ToolResultMessage $message): float
    {
        // Count role characters
        $chars = mb_strlen($message->getRole());
        $chars = array_reduce($message->getTools(), function (int $carry, ToolInterface $tool): int {
