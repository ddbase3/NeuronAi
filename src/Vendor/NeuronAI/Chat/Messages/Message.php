...
class Message implements JsonSerializable
{
     ...
     */
    public function __construct(protected MessageRole $role, string|ContentBlockInterface|array|null $content = null)
    {
        if ($content !== null) {
    ...
    {
        $text = implode(' ', array_map(fn(TextContent $block): string => $block->content, array_filter($this->getContentBlocks(), fn(ContentBlockInterface $block): bool => $block instanceof TextContent && !$block instanceof ReasoningContent)));
    ...
    {
        $data = ['role' => $this->getRole(), 'content' => array_map(fn(ContentBlockInterface $block): array => $block->toArray(), $this->contents)];
