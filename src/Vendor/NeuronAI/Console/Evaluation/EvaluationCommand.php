...
namespace NeuronAi\Vendor\NeuronAI\Console\Evaluation;

use NeuronAi\Vendor\NeuronAI\Evaluation\BaseEvaluator;
use NeuronAi\Vendor\NeuronAI\Evaluation\Config\ConfigLoader;
use NeuronAi\Vendor\NeuronAI\Evaluation\Config\EvaluationOutputResolver;
use NeuronAi\Vendor\NeuronAI\Evaluation\Discovery\EvaluatorDiscovery;
use NeuronAi\Vendor\NeuronAI\Evaluation\Output\OutputPipeline;
use NeuronAi\Vendor\NeuronAI\Evaluation\Runner\EvaluatorSummary;
use NeuronAi\Vendor\NeuronAI\Evaluation\Runner\EvaluatorRunner;
use ReflectionClass;
...
class EvaluationCommand
{
    ...
    public function __construct(private readonly ?ConfigLoader $configLoader = new ConfigLoader(), private readonly ?EvaluationOutputResolver $driverResolver = new EvaluationOutputResolver())
    {
    ...
    {
        $options = ['path' => '', 'verbose' => \false, 'help' => \false];
        // Skip script name
        array_shift($args);
        foreach ($args as $arg) {
            if ($arg === '--help' || $arg === '-h') {
    ...
    private function createEvaluator(string $className): BaseEvaluator
    {
        try {
            ...
            throw new RuntimeException("Evaluator {$className} requires constructor parameters. " . "Please ensure evaluators can be instantiated without arguments.");
