...
class MessageMapper implements MessageMapperInterface
{
    ...
    {
        return ['role' => $message->getRole(), 'content' => $this->mapBlocks($message->getContentBlocks())];
    }
    ...
    protected function mapSingleBlock(ContentBlockInterface $block): ?array
    {
        return match ($block::class) {
            ...
            ReasoningContent::class => ['type' => 'thinking', 'thinking' => $block->content, 'signature' => $block->id],
    ...
    protected function mapImageBlock(ImageContent $block): ?array
    {
        return match ($block->sourceType) {
            SourceType::URL => ['type' => 'image', 'source' => ['type' => 'url', 'url' => $block->content]],
            SourceType::BASE64 => ['type' => 'image', 'source' => ['type' => 'base64', 'media_type' => $block->mediaType, 'data' => $block->content]],
            SourceType::ID => ['type' => 'image', 'source' => ['type' => 'file', 'file_id' => $block->content]],
        };
    }
    protected function mapFileBlock(FileContent $block): array
    {
        return match ($block->sourceType) {
            SourceType::URL => ['type' => 'document', 'source' => ['type' => 'url', 'url' => $block->content]],
            SourceType::BASE64 => ['type' => 'document', 'source' => ['type' => 'base64', 'media_type' => $block->mediaType, 'data' => $block->content]],
            SourceType::ID => ['type' => 'document', 'source' => ['type' => 'file', 'file_id' => $block->content]],
        };
    }
    protected function mapToolCall(ToolCallMessage $message): array
    {
        ...
        foreach ($message->getTools() as $tool) {
            $parts[] = ['type' => 'tool_use', 'id' => $tool->getCallId(), 'name' => $tool->getName(), 'input' => $tool->getInputs() ?: new stdClass()];
        }
        return ['role' => MessageRole::ASSISTANT, 'content' => $parts];
    }
    protected function mapToolsResult(ToolResultMessage $message): array
    {
        $parts = array_map(fn(ToolInterface $tool): array => ['type' => 'tool_result', 'tool_use_id' => $tool->getCallId(), 'content' => $tool->getResult()], $message->getTools());
        if ($contentBlocks = $message->getContentBlocks()) {
            $parts = [...$parts, ...$this->mapBlocks($contentBlocks)];
        }
        return ['role' => MessageRole::USER, 'content' => array_values($parts)];
    }
}
