...
trait HandleStream
{
    ...
    {
        $json = ['stream' => \true, 'model' => $this->model, 'max_tokens' => $this->max_tokens, 'messages' => $this->messageMapper()->map($messages), ...$this->parameters];
        ...
        $stream = $this->httpClient->stream(HttpRequest::post(uri: 'messages', body: $json));
        ...
        if ($this->streamState->hasToolCalls()) {
            return $this->createToolCallMessage($this->streamState->getToolCalls(), $this->streamState->getContentBlocks())->setUsage($this->streamState->getUsage())->addMetadata('cacheWriteTokens', (string) $this->streamState->getCacheWriteTokens())->addMetadata('cacheReadTokens', (string) $this->streamState->getCacheReadTokens());
        }
        $message = new AssistantMessage($this->streamState->getContentBlocks());
        $message->setUsage($this->streamState->getUsage())->addMetadata('cacheWriteTokens', (string) $this->streamState->getCacheWriteTokens())->addMetadata('cacheReadTokens', (string) $this->streamState->getCacheReadTokens());
    ...
    protected function handleMessageStart(array $message): void
    {
        ...
        $this->streamState->addCacheWriteTokens(($cacheCreation['ephemeral_5m_input_tokens'] ?? 0) + ($cacheCreation['ephemeral_1h_input_tokens'] ?? 0) + ($message['usage']['cache_creation_input_tokens'] ?? 0));
