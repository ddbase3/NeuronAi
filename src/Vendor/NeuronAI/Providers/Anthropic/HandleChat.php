...
trait HandleChat
{
    ...
    {
        $json = ['model' => $this->model, 'max_tokens' => $this->max_tokens, 'messages' => $this->messageMapper()->map($messages), ...$this->parameters];
        ...
        $response = $this->httpClient->request(HttpRequest::post(uri: 'messages', body: $json));
    ...
    protected function processChatResult(array $result): AssistantMessage
    {
        ...
        if (isset($result['usage'])) {
            ...
            $cacheWrite = ($cacheCreation['ephemeral_5m_input_tokens'] ?? 0) + ($cacheCreation['ephemeral_1h_input_tokens'] ?? 0) + ($usage['cache_creation_input_tokens'] ?? 0);
            ...
            if ($cacheWrite > 0 || $cacheRead > 0) {
                $message->addMetadata('cacheWriteTokens', (string) $cacheWrite)->addMetadata('cacheReadTokens', (string) $cacheRead);
            }
        }
