...
class MessageMapper implements MessageMapperInterface
{
    ...
    public function mapMessage(Message $message): void
    {
        ...
        $payload = ['role' => $message->getRole(), 'content' => $textContent];
        if ($images !== []) {
    ...
    {
        $payload = ['role' => $message->getRole(), 'content' => $message->getContent()];
    ...
    public function mapToolsResult(ToolResultMessage $message): void
    {
        foreach ($message->getTools() as $tool) {
            $this->mapping[] = ['role' => MessageRole::TOOL->value, 'content' => $tool->getResult()];
        }
    }
}
