...
trait HandleStream
{
    ...
    public function stream(Message ...$messages): Generator
    {
        ...
        $body = ['stream' => \true, 'model' => $this->model, 'messages' => $this->messageMapper()->map($messages), ...$this->parameters];
        if (!empty($this->tools)) {
            $body['tools'] = $this->toolPayloadMapper()->map($this->tools);
        }
        $stream = $this->httpClient->stream(HttpRequest::post(uri: 'chat', body: $body));
        ...
        while (!$stream->eof()) {
            ...
            if (isset($line['message']['tool_calls'])) {
                return $this->createToolCallMessage($line['message']['tool_calls'], $this->streamState->getContentBlocks())->setUsage($this->streamState->getUsage());
            }
