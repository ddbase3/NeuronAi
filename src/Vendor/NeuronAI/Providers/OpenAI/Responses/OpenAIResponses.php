...
class OpenAIResponses implements AIProviderInterface
{
    ...
    {
        $tools = array_map(fn(array $item): ToolInterface => $this->findTool($item['name'])->setInputs(json_decode((string) $item['arguments'], \true))->setCallId($item['call_id']), $toolCalls);
    ...
    protected function extractCitations(string $text, array $annotations): array
    {
        ...
        foreach ($annotations as $annotation) {
            ...
            if ($type === 'file_citation') {
                ...
                $citations[] = new Citation(id: $fileCitation['file_id'] ?? uniqid('openai_responses_file_'), source: $fileCitation['file_id'] ?? '', startIndex: $annotation['start_index'] ?? null, endIndex: $annotation['end_index'] ?? null, citedText: $annotation['text'] ?? null, metadata: ['type' => 'file_citation', 'quote' => $fileCitation['quote'] ?? null, 'provider' => 'openai_responses']);
            } elseif ($type === 'file_path') {
                $filePath = $annotation['file_path'] ?? [];
                $citations[] = new Citation(id: $filePath['file_id'] ?? uniqid('openai_responses_path_'), source: $filePath['file_id'] ?? '', startIndex: $annotation['start_index'] ?? null, endIndex: $annotation['end_index'] ?? null, citedText: $annotation['text'] ?? null, metadata: ['type' => 'file_path', 'provider' => 'openai_responses']);
            }
        }
        return $citations;
    }
