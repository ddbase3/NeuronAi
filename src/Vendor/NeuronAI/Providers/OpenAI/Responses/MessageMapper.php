...
class MessageMapper implements MessageMapperInterface
{
    ...
    {
        $this->mapping[] = ['role' => $message->getRole(), 'content' => $this->mapBlocks($message->getContentBlocks(), $this->isUserMessage($message))];
    }
    /**
    ...
    {
        return array_values(array_filter(array_map(fn(ContentBlockInterface $item): ?array => $this->mapContentBlock($item, $isUser), $blocks)));
    }
    ...
    {
        return ['type' => $forUser ? 'input_text' : 'output_text', 'text' => $block->content];
    }
    protected function mapFileBlock(FileContent $block): array
    {
        return match ($block->sourceType) {
            SourceType::BASE64 => ['type' => 'input_file', 'filename' => $block->filename, 'file_data' => "data:{$block->mediaType};base64,{$block->content}"],
            SourceType::URL => ['type' => 'input_file', 'file_url' => $block->content],
            SourceType::ID => ['type' => 'input_file', 'file_id' => $block->content],
        };
    }
    protected function mapImageBlock(ImageContent $block): array
    {
        return match ($block->sourceType) {
            SourceType::URL => ['type' => 'input_image', 'image_url' => $block->content],
            SourceType::BASE64 => ['type' => 'input_image', 'image_url' => 'data:' . $block->mediaType . ';base64,' . $block->content],
            SourceType::ID => ['type' => 'input_image', 'file_id' => $block->content],
        };
    }
    ...
    protected function mapToolCall(ToolCallMessage $message): void
    {
        ...
        if ($contentBlocks = $message->getContentBlocks()) {
            ...
            if ($blocks !== []) {
                $this->mapping[] = ['role' => $message->getRole(), 'content' => $blocks];
            }
        }
        // Add function call items
        foreach ($message->getTools() as $tool) {
            $this->mapping[] = ['type' => 'function_call', 'name' => $tool->getName(), 'arguments' => json_encode($tool->getInputs() ?: new stdClass()), 'call_id' => $tool->getCallId()];
        }
    }
    protected function mapToolsResult(ToolResultMessage $message): void
    {
        foreach ($message->getTools() as $tool) {
            $this->mapping[] = ['type' => 'function_call_output', 'call_id' => $tool->getCallId(), 'output' => $tool->getResult()];
        }
    }
}
