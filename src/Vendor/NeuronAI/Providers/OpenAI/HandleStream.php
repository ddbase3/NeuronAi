...
trait HandleStream
{
    ...
    public function stream(Message ...$messages): Generator
    {
        ...
        $body = ['stream' => \true, 'model' => $this->model, 'messages' => $this->messageMapper()->map($messages), 'stream_options' => ['include_usage' => \true], ...$this->parameters];
        // Attach tools
        if (!empty($this->tools)) {
            $body['tools'] = $this->toolPayloadMapper()->map($this->tools);
        }
        $stream = $this->httpClient->stream($this->createChatHttpRequest($body));
    ...
    protected function processStream(StreamInterface $stream): Generator
    {
        while (!$stream->eof()) {
            ...
            if ($this->finishForToolCall($choice)) {
                toolcall:
                yield from $this->processToolCallDelta($choice);
                $message = $this->createToolCallMessage($this->streamState->getToolCalls(), $this->streamState->getContentBlocks());
