...
class OpenAITextToSpeech implements AIProviderInterface
{
    ...
    public function __construct(protected string $key, protected string $model, protected string $voice, protected array $parameters = [], ?HttpClientInterface $httpClient = null)
    {
    ...
    public function chat(Message ...$messages): Message
    {
        ...
        $json = ['model' => $this->model, 'input' => $message->getContent(), 'voice' => $this->voice, 'instructions' => $this->system ?? '', ...$this->parameters];
        $response = $this->httpClient->request(HttpRequest::post(uri: 'audio/speech', body: $json));
    ...
    {
        $message = end($messages);
        $json = ['stream' => \true, 'model' => $this->model, 'input' => $message->getContent(), 'voice' => $this->voice, 'instructions' => $this->system ?? '', ...$this->parameters];
        $stream = $this->httpClient->stream(HttpRequest::post(uri: 'audio/speech', body: $json));
        $content = '';
        $usage = new Usage(0, 0);
        ...
        $message = new AssistantMessage(new AudioContent($content, SourceType::BASE64));
