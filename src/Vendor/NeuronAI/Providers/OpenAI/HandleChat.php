...
trait HandleChat
{
    ...
    public function chat(Message ...$messages): Message
    {
        ...
        $response = $this->httpClient->request($this->createChatHttpRequest($body));
    ...
    protected function processChatResult(array $result): AssistantMessage
    {
        if ($result['choices'][0]['finish_reason'] === 'tool_calls') {
            $block = isset($result['choices'][0]['message']['content']) ? new TextContent($result['choices'][0]['message']['content']) : null;
        ...
        if (isset($result['usage'])) {
            $response->setUsage(new Usage($result['usage']['prompt_tokens'] ?? 0, $result['usage']['completion_tokens'] ?? 0, $result['usage']['prompt_tokens_details']['cached_tokens'] ?? 0, $result['usage']['completion_tokens_details']['reasoning_tokens'] ?? 0));
        }
    ...
    protected function processAnnotation(array $annotation): ?Citation
    {
        ...
        if ($type === 'file_citation') {
            ...
            return new Citation(id: $fileCitation['file_id'] ?? uniqid('openai_file_'), source: $fileCitation['file_id'] ?? '', startIndex: $annotation['start_index'] ?? null, endIndex: $annotation['end_index'] ?? null, citedText: $annotation['text'] ?? null, metadata: ['type' => 'file_citation', 'quote' => $fileCitation['quote'] ?? null, 'provider' => 'openai']);
        }
        if ($type === 'file_path') {
            $filePath = $annotation['file_path'] ?? [];
            return new Citation(id: $filePath['file_id'] ?? uniqid('openai_path_'), source: $filePath['file_id'] ?? '', startIndex: $annotation['start_index'] ?? null, endIndex: $annotation['end_index'] ?? null, citedText: $annotation['text'] ?? null, metadata: ['type' => 'file_path', 'provider' => 'openai']);
        }
        return null;
    }
}
