...
class Cohere extends OpenAI
{
     ...
     */
    public function __construct(string $key, string $model, protected string $baseUri = 'https://api.cohere.ai/v2', array $parameters = [], bool $strict_response = \false, ?HttpClientInterface $httpClient = null)
    {
    ...
    protected function createChatHttpRequest(array $payload): HttpRequest
    {
        ...
        return HttpRequest::post(uri: 'chat', body: $payload);
    }
    public function structured(Message|array $messages, string $class, array $response_format): Message
    {
        $this->parameters = array_replace_recursive($this->parameters, ['response_format' => ['type' => 'json_object', 'json_schema' => $response_format]]);
