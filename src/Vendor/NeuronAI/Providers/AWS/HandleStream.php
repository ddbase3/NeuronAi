...
use NeuronAi\Vendor\Aws\Api\Parser\EventParsingIterator;
use Generator;
        ...
        foreach ($result as $eventParserIterator) {
            ...
            foreach ($eventParserIterator as $event) {
                if (isset($event['metadata'])) {
                    ...
                    $this->streamState->addCachedInputTokens($event['metadata']['usage']['cacheReadInputTokens'] ?? 0);
                }
                ...
                if (isset($event['contentBlockDelta']['delta']['text'])) {
                    ...
                    $this->streamState->updateContentBlock($event['contentBlockDelta']['contentBlockIndex'], new TextContent($textChunk));
                ...
                if (isset($event['contentBlockDelta']['delta']['reasoningContent'])) {
                    ...
                    $this->streamState->updateContentBlock($event['contentBlockDelta']['contentBlockIndex'], new ReasoningContent($reasoningChunk, $event['contentBlockDelta']['delta']['reasoningContent']['signature']));
