...
namespace NeuronAi\Vendor\NeuronAI\Providers\ZAI\Image;
...
class ZAIImage implements AIProviderInterface
{
    ...
    public function chat(Message ...$messages): Message
    {
        ...
        $body = ['model' => $this->model, 'prompt' => $message->getContent(), ...$this->parameters];
        $response = $this->httpClient->request(HttpRequest::post(uri: 'images/generations', body: $body))->json();
        $result = new AssistantMessage(new ImageContent($response['data'][0]['url'], SourceType::URL));
        if (isset($response['usage'])) {
            $result->setUsage(new Usage($response['usage']['prompt_tokens'] ?? 0, $response['usage']['completion_tokens'] ?? 0));
        }
        return $result;
    }
