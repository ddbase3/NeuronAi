...
trait HandleStructured
{
     ...
     */
    protected array $unsupportedModels = ['gemini-2.0-flash', 'gemini-2.0-flash-lite-preview', 'gemini-2.0-pro-preview', 'gemini-2.0-flash-thinking-preview', 'gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-1.0-pro'];
    /**
        ...
        try {
            ...
            if (!empty($this->tools) && in_array($this->model, $this->unsupportedModels)) {
                ...
                if ($last_message instanceof Message && $last_message->getRole() === MessageRole::USER->value) {
                    $last_message->setContents($last_message->getContent() . ' Respond using this JSON schema: ' . json_encode($response_format));
                }
            } else {
