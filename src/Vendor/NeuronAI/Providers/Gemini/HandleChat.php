...
trait HandleChat
{
    ...
    public function chat(Message ...$messages): Message
    {
        ...
        $response = $this->httpClient->request(HttpRequest::post(uri: "{$this->model}:generateContent", body: $body));
    ...
    protected function processChatResult(array $result): AssistantMessage
    {
        ...
        foreach ($content['parts'] as $part) {
            if (isset($part['text'])) {
                $block = $part['thought'] ?? \false ? new ReasoningContent($part['text']) : new TextContent($part['text']);
            ...
            if (isset($part['inlineData'])) {
                $block = new ImageContent($part['inlineData']['data'], SourceType::BASE64, $part['inlineData']['mimeType']);
                if (isset($part['thoughtSignature'])) {
                    $block->addMetadata('thought_signature', $part['thoughtSignature']);
                }
                $blocks[] = $block;
        ...
        if (array_key_exists('usageMetadata', $result)) {
            $message->setUsage(new Usage($result['usageMetadata']['promptTokenCount'], $result['usageMetadata']['candidatesTokenCount'] ?? 0, $result['usageMetadata']['cachedContentTokenCount'] ?? 0, $result['usageMetadata']['thoughtsTokenCount'] ?? 0));
        }
