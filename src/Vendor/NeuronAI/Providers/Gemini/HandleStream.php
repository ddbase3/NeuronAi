...
trait HandleStream
{
    ...
    {
        $body = ['contents' => $this->messageMapper()->map($messages), ...$this->parameters];
        ...
        $stream = $this->httpClient->stream(HttpRequest::post(uri: "{$this->model}:streamGenerateContent", body: $body));
        $this->streamState = new StreamState();
        $lastFinishReason = null;
        while (!$stream->eof()) {
            $line = $this->readLine($stream);
            if (($line = json_decode((string) $line, \true)) === null) {
                continue;
            }
            ...
            if (array_key_exists('usageMetadata', $line) && array_key_exists('promptTokenCount', $line['usageMetadata']) && array_key_exists('candidatesTokenCount', $line['usageMetadata'])) {
            ...
            if (isset($line['candidates'][0]['finishReason']) && $line['candidates'][0]['finishReason'] === 'STOP' && $this->streamState->hasToolCalls()) {
                toolcall:
                return $this->createToolCallMessage($this->streamState->getContentBlocks(), $this->streamState->getToolCalls())->setUsage($this->streamState->getUsage());
            }
            ...
            if (isset($part['inlineData'])) {
                $this->streamState->addContentBlock('image', new ImageContent($part['inlineData']['data'], SourceType::BASE64, $part['inlineData']['mimeType']));
                continue;
            }
            if (isset($part['fileData'])) {
                $this->streamState->addContentBlock('file', new FileContent($part['fileData']['fileUri'], SourceType::URL, $part['fileData']['mimeType']));
            }
        }
