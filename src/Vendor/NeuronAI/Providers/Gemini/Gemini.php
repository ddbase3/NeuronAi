...
class Gemini implements AIProviderInterface
{
     ...
     */
    public function __construct(protected string $key, protected string $model, protected array $parameters = [], ?HttpClientInterface $httpClient = null, protected string $baseUri = 'https://generativelanguage.googleapis.com/v1beta/models')
    {
    ...
    protected function createToolCallMessage(array $blocks, array $toolCalls): ToolCallMessage
    {
        $tools = array_map(function (array $item): ToolInterface {
            return $this->findTool($item['functionCall']['name'])->setInputs($item['functionCall']['args'])->setCallId($item['functionCall']['name']);
            // Gemini uses the tool's name as a unique identifier.
        }, $toolCalls);
    ...
    protected function extractCitations(array $groundingMetadata): array
    {
        ...
        if (isset($groundingMetadata['groundingChunks'])) {
            foreach ($groundingMetadata['groundingChunks'] as $index => $chunk) {
                if (isset($chunk['web'])) {
                    $citations[] = new Citation(id: 'gemini_chunk_' . $index, source: $chunk['web']['uri'] ?? '', title: $chunk['web']['title'] ?? null, metadata: ['chunk_index' => $index, 'provider' => 'gemini']);
                }
            }
        }
            ...
            foreach ($groundingMetadata['groundingSupports'] as $support) {
                ...
                foreach ($chunkIndices as $idx => $chunkIndex) {
                    ...
                    if ($sourceChunk && isset($sourceChunk['web'])) {
                        $citations[] = new Citation(id: 'gemini_support_' . uniqid(), source: $sourceChunk['web']['uri'] ?? '', title: $sourceChunk['web']['title'] ?? null, startIndex: $segment['startIndex'] ?? null, endIndex: $segment['endIndex'] ?? null, citedText: $segment['text'] ?? null, metadata: ['chunk_index' => $chunkIndex, 'confidence' => $confidenceScores[$idx] ?? null, 'provider' => 'gemini']);
                    }
                }
            }
        }
