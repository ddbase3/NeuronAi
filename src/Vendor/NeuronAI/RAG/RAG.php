...
use NeuronAi\Vendor\NeuronAI\RAG\Nodes\InstructionsNode;
use NeuronAi\Vendor\NeuronAI\RAG\Nodes\PostProcessNode;
use NeuronAi\Vendor\NeuronAI\RAG\Nodes\PreProcessNode;
use NeuronAi\Vendor\NeuronAI\RAG\Nodes\RetrievalNode;
use NeuronAi\Vendor\NeuronAI\RAG\PostProcessor\PostProcessorInterface;
use NeuronAi\Vendor\NeuronAI\RAG\PreProcessor\PreProcessorInterface;
...
class RAG extends Agent
{
    ...
    {
        return [new PreProcessNode($this->preProcessors()), new RetrievalNode($this->resolveRetrieval()), new PostProcessNode($this->postProcessors()), new InstructionsNode($this->resolveInstructions(), $this->bootstrapTools())];
    }
    /**
    ...
    public function addDocuments(array $documents, int $chunkSize = 50): void
    {
        foreach (array_chunk($documents, $chunkSize) as $chunk) {
            $this->resolveVectorStore()->addDocuments($this->resolveEmbeddingsProvider()->embedDocuments($chunk));
        }
    }
    /**
