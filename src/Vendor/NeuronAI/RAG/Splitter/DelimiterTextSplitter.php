...
class DelimiterTextSplitter extends AbstractSplitter
{
    ...
    public function __construct(private readonly int $maxLength = 1000, private readonly string $separator = ' ', private readonly int $wordOverlap = 0, private readonly int $minLength = 0)
    {
        if ($maxLength <= 0) {
    ...
    private function calculateChunkLength(array $chunk): int
    {
        ...
        return array_sum(array_map(mb_strlen(...), $chunk)) + (count($chunk) - 1) * $this->separatorLength;
    }
}
