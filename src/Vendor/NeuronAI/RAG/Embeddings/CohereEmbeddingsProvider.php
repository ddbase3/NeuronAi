...
class CohereEmbeddingsProvider extends AbstractEmbeddingsProvider
{
    ...
    {
        $response = $this->httpClient->request(HttpRequest::post('embed', ['model' => $this->model, 'texts' => [$text], 'embedding_types' => ['float'], ...array_merge(['input_type' => 'search_query'], $this->parameters)]))->json();
    ...
    public function embedDocuments(array $documents): array
    {
        ...
        foreach ($chunks as $chunk) {
            $response = $this->httpClient->request(HttpRequest::post('embed', ['model' => $this->model, 'texts' => array_map(fn(Document $document): string => $document->getContent(), $chunk), 'embedding_types' => ['float'], ...array_merge(['input_type' => 'search_query'], $this->parameters)]))->json();
