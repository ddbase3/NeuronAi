...
use NeuronAi\Vendor\NeuronAI\Agent\SystemPrompt;
class QueryTransformationPreProcessor implements PreProcessorInterface
{
     ...
     */
    public function __construct(protected AIProviderInterface $provider, protected QueryTransformationType $transformation = QueryTransformationType::REWRITING, protected ?string $customPrompt = null)
    {
    }
    /**
    ...
    public function process(Message $question): Message
    {
        ...
        return $this->provider->systemPrompt($this->getSystemPrompt())->chat($preparedMessage);
    }
    ...
    {
        return (string) new SystemPrompt(background: ['You are an AI assistant tasked with reformulating user queries to improve retrieval in a RAG system.'], steps: ['Given the original query, rewrite it to be more specific, detailed, and likely to retrieve relevant information.', 'Focus on expanding vocabulary and technical terminology while preserving the original intent and meaning.'], output: ['Output only the reformulated query', 'Do not add temporal references, dates, or years unless they are present in the original query']);
    }
    protected function getDecompositionPrompt(): string
    {
        return (string) new SystemPrompt(background: ['You are an AI assistant that breaks down complex queries into simpler sub-queries for comprehensive information retrieval in a RAG system.'], steps: ['Given the original complex query, decompose it into focused sub-queries.', 'Each sub-query should address a specific aspect of the original question.', 'Ensure all sub-queries together cover the full scope of the original query.'], output: ['Output each sub-query on a separate line', 'Use clear, specific language for each sub-query', 'Do not add temporal references, dates, or years unless they are present in the original query']);
    }
    protected function getHydePrompt(): string
    {
        return (string) new SystemPrompt(background: ['You are an AI assistant that generates hypothetical answer to the user query, to improve retrieval in a RAG system.'], steps: ['Given the original query, write a hypothetical document passage that would directly answer this question.', 'Create content that resembles what you would expect to find in a relevant document.'], output: ['Output only the hypothetical document passage', 'Do not add temporal references, dates, or years unless they are present in the original query', 'Keep the response as concise as possible']);
    }
}
