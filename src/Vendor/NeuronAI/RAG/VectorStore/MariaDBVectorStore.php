...
{
    public function __construct(protected PDO $pdo, protected string $tableName = 'rag_documents', protected int $topK = 4)
    {
    }
    /**
...
, $this->tableName, $dimensions));
    ...
    public function addDocuments(array $documents): VectorStoreInterface
    {
        ...
        $stmt = $this->pdo->prepare(sprintf(<<<'SQL'
...
, $this->tableName));
        ...
        foreach ($chunks as $chunk) {
            foreach ($chunk as $document) {
                ...
                $stmt->execute([':id' => $document->getId(), ':content' => $document->getContent(), ':sourceType' => $document->getSourceType(), ':sourceName' => $document->getSourceName(), ':metadata' => json_encode($document->metadata), ':embedding' => json_encode($document->getEmbedding())]);
            }
        }
        return $this;
    }
    ...
    {
        if ($sourceName !== null) {
            $stmt = $this->pdo->prepare(sprintf('DELETE FROM %s WHERE sourceType = :sourceType AND sourceName = :sourceName', $this->tableName));
            $stmt->execute([':sourceType' => $sourceType, ':sourceName' => $sourceName]);
        } else {
            $stmt = $this->pdo->prepare(sprintf('DELETE FROM %s WHERE sourceType = :sourceType', $this->tableName));
...
, $this->tableName, $this->topK));
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $metadata = json_decode($row['metadata'] ?? '{}', \true);
