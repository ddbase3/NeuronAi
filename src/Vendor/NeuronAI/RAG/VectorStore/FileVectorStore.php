...
{
    public function __construct(protected string $directory, protected int $topK = 4, protected string $name = 'neuron', protected string $ext = '.store')
    {
        if (!is_dir($this->directory) && !@mkdir($this->directory, 0755, \true)) {
    ...
    {
        $this->appendToFile(array_map(fn(Document $document): array => $document->jsonSerialize(), $documents));
        return $this;
    }
    /**
    ...
    public function deleteBy(string $sourceType, ?string $sourceName = null): VectorStoreInterface
    {
        ...
        try {
            foreach ($this->getLine($this->getFilePath()) as $line) {
                $document = json_decode((string) $line, \true);
    ...
    public function similaritySearch(array $embedding): array
    {
        ...
        foreach ($this->getLine($this->getFilePath()) as $document) {
            $document = json_decode((string) $document, \true);
            ...
            if (count($topItems) > $this->topK) {
                $topItems = array_slice($topItems, 0, $this->topK, \true);
            }
        }
    ...
    {
        file_put_contents($this->getFilePath(), implode(PHP_EOL, array_map(json_encode(...), $documents)) . PHP_EOL, FILE_APPEND);
    }
