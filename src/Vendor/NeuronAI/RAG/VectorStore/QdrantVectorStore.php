...
class QdrantVectorStore implements VectorStoreInterface
{
    ...
    public function __construct(
        ...
        // like http://localhost:6333/collections/neuron-ai/
    ...
    {
        $this->httpClient = ($httpClient ?? new GuzzleHttpClient())->withBaseUri($this->collectionUrl)->withHeaders(['Content-Type' => 'application/json', ...!is_null($this->key) && $this->key !== '' ? ['api-key' => $this->key] : []]);
        $this->initialize();
    }
    /**
     * @throws HttpException
    ...
    {
        $response = $this->httpClient->request(HttpRequest::get(uri: 'exists'))->json();
    ...
    {
        $points = array_map(fn(Document $document): array => ['id' => (string) $document->getId(), 'payload' => ['content' => $document->getContent(), 'sourceType' => $document->getSourceType(), 'sourceName' => $document->getSourceName(), ...$document->metadata], 'vector' => $document->getEmbedding()], $documents);
        $chunks = array_chunk($points, 100);
        foreach ($chunks as $chunk) {
            $this->httpClient->request(HttpRequest::put(uri: 'points?wait=true', body: ['points' => $chunk]));
        }
        return $this;
    }
    /**
    ...
    public function deleteBy(string $sourceType, ?string $sourceName = null): VectorStoreInterface
    {
        ...
        if ($sourceName !== null) {
            $must[] = ['key' => 'sourceName', 'match' => ['value' => $sourceName]];
        }
        $this->httpClient->request(HttpRequest::post(uri: 'points/delete?wait=true', body: ['filter' => ['must' => $must]]));
        return $this;
    }
    /**
    ...
    {
        $body = ['query' => ['recommend' => ['positive' => [$embedding]]], 'limit' => $this->topK, 'with_payload' => \true, 'with_vector' => \true];
        if ($this->filters !== []) {
            $body['filter'] = ['must' => $this->filters];
        }
        $response = $this->httpClient->request(HttpRequest::post(uri: 'points/query', body: $body))->json();
    ...
    {
        $this->httpClient->request(HttpRequest::put(uri: '', body: ['vectors' => ['size' => $this->dimension, 'distance' => 'Cosine']]));
    }
}
