...
use NeuronAi\Vendor\Http\Client\Exception;
use NeuronAi\Vendor\NeuronAI\RAG\Document;
use NeuronAi\Vendor\NeuronAI\RAG\VectorSimilarity;
use NeuronAi\Vendor\Typesense\Client;
use NeuronAi\Vendor\Typesense\Exceptions\ObjectNotFound;
use NeuronAi\Vendor\Typesense\Exceptions\TypesenseClientError;
use JsonException;
...
{
    public function __construct(protected Client $client, protected string $collection, protected int $vectorDimension, protected string $topK = '4')
    {
    }
    /**
     * @throws Exception
    ...
    public function checkIndexStatus(Document $document): void
    {
        try {
            ...
            $fields = [['name' => 'content', 'type' => 'string'], ['name' => 'sourceType', 'type' => 'string', 'facet' => \true], ['name' => 'sourceName', 'type' => 'string', 'facet' => \true], ['name' => 'embedding', 'type' => 'float[]', 'num_dim' => $this->vectorDimension]];
            // Map custom fields
            foreach ($document->metadata as $name => $value) {
                $fields[] = ['name' => $name, 'type' => gettype($value), 'facet' => \true];
            }
            $this->client->collections->create(['name' => $this->collection, 'fields' => $fields]);
        }
    }
    /**
     * @throws Exception
    ...
    {
        $filter = $sourceName !== null ? "sourceType:={$sourceType} && sourceName:={$sourceName}" : "sourceType:={$sourceType}";
    ...
    {
        $params = ['collection' => $this->collection, 'q' => '*', 'vector_query' => 'embedding:(' . json_encode($embedding) . ')', 'exclude_fields' => 'embedding', 'per_page' => $this->topK, 'num_candidates' => max(50, intval($this->topK) * 4)];
    ...
    private function checkVectorDimension(int $dimension): void
    {
        ...
        if (array_key_exists('num_dim', $embeddingField) && $embeddingField['num_dim'] === $dimension) {
            return;
        }
        throw new \Exception("Vector embeddings dimension {$dimension} must be the same as the initial setup {$this->vectorDimension} - " . json_encode($embeddingField));
    }
}
