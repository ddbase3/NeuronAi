...
namespace NeuronAi\Vendor\NeuronAI\RAG\GraphStore;

use NeuronAi\Vendor\Laudis\Neo4j\Authentication\Authenticate;
use NeuronAi\Vendor\Laudis\Neo4j\ClientBuilder;
use NeuronAi\Vendor\Laudis\Neo4j\Contracts\ClientInterface;
use Exception;
...
class Neo4jGraphStore implements GraphStoreInterface
{
    ...
    public function __construct(protected string $uri = 'bolt://localhost:7687', protected string $username = '', protected string $password = '', protected string $database = 'neo4j', protected string $nodeLabel = 'Entity')
    {
    }
...
CYPHER;
        $this->client()->run($query, ['subject' => $subject, 'object' => $object]);
...
DELETE n
CYPHER;
        $this->client()->run($cleanupQuery, ['subject' => $subject, 'object' => $object]);
        // Invalidate schema cache
        $this->cachedSchema = null;
    }
...
CYPHER;
        ...
        foreach ($result as $record) {
            $triplets[] = new Triplet($subject, $record->get('relation'), $record->get('object'));
        }
        return $triplets;
    }
...
CYPHER;
    ...
    {
        return $this->client ?? $this->client = ClientBuilder::create()->withDriver('default', $this->uri, Authenticate::basic($this->username, $this->password))->withDefaultDriver('default')->build();
    }
}
