...
class ObjectProperty implements ToolPropertyInterface
{
     ...
     */
    public function __construct(protected string $name, protected ?string $description = null, protected bool $required = \false, protected ?string $class = null, protected array $properties = [])
    {
    ...
    protected function createPropertyFromSchema(string $propertyName, array $propertyData, bool $isRequired): ?ToolPropertyInterface
    {
        ...
        return match ($type) {
            ...
            default => new ToolProperty($propertyName, PropertyType::STRING, $description, $isRequired, $propertyData['enum'] ?? []),
        };
    }
    /**
    ...
    protected function createObjectProperty(string $name, array $propertyData, bool $required, ?string $description): ObjectProperty
    {
        ...
        return new ObjectProperty($name, $description, $required, $className, $nestedProperties);
    }
    /**
    ...
    protected function createArrayProperty(string $name, array $propertyData, bool $required, ?string $description): ArrayProperty
    {
        ...
        return new ArrayProperty($name, $description, $required, $items, $minItems, $maxItems);
    }
    /**
    ...
    {
        return new ToolProperty($name, PropertyType::fromSchema($propertyData['type']), $description, $required, $propertyData['enum'] ?? []);
    }
    public function jsonSerialize(): array
    {
        return ['name' => $this->name, ...is_null($this->description) ? [] : ['description' => $this->description], 'type' => $this->type, 'properties' => $this->getJsonSchema(), 'required' => $this->required];
    }
    // The mapped class required properties and required properties are merged
    public function getRequiredProperties(): array
    {
        return array_values(array_filter(array_map(fn(ToolPropertyInterface $property): ?string => $property->isRequired() ? $property->getName() : null, $this->properties)));
    }
