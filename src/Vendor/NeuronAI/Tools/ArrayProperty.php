...
class ArrayProperty implements ToolPropertyInterface
{
     ...
     */
    public function __construct(protected string $name, protected ?string $description = null, protected bool $required = \false, protected ?ToolPropertyInterface $items = null, protected ?int $minItems = null, protected ?int $maxItems = null)
    {
        $this->validateConstraints();
    }
    public function jsonSerialize(): array
    {
        return ['name' => $this->name, ...$this->getJsonSchema(), 'required' => $this->required];
    }
    ...
    protected function validateConstraints(): void
    {
        ...
        if ($this->minItems !== null && $this->maxItems !== null && $this->minItems > $this->maxItems) {
            throw new ArrayPropertyException("minItems ({$this->minItems}) cannot be greater than maxItems ({$this->maxItems})");
        }
    }
}
