...
class TavilyCrawlTool extends Tool
{
    ...
    protected array $options = ['include_images' => \false, 'allow_external' => \false];
    /**
     * @param string $key Tavily API key.
     */
    public function __construct(protected string $key)
    {
        parent::__construct('url_crawl', 'Get the entire website in markdown format.');
    }
    protected function properties(): array
    {
        return [new ToolProperty('url', PropertyType::STRING, 'The URL to crawl.', \true)];
    }
    public function __invoke(string $url): array
    {
        ...
        $result = $this->getClient()->post('crawl', [RequestOptions::JSON => array_merge($this->options, ['url' => $url])]);
        return json_decode((string) $result->getBody(), \true);
    }
