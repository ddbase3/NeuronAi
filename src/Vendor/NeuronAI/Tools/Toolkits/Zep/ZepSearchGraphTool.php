...
class ZepSearchGraphTool extends Tool
{
    ...
    {
        parent::__construct('search_knowledge_graph', 'Searches the knowledge graph for relevant facts or nodes.
Use this tool if you need to retrieve user information that can help you provide more accurate answers.');
        $this->createUser();
    }
    protected function properties(): array
    {
        return [new ToolProperty('query', PropertyType::STRING, 'The search term to find relevant facts or nodes', \true), new ToolProperty('search_scope', PropertyType::STRING, 'The scope of the search to perform. Can be "facts" or "nodes"', \false, ['facts', 'nodes'])];
    }
    public function __invoke(string $query, string $search_scope = 'facts', int $limit = 5): array
    {
        $response = $this->getClient()->post('graph/search', [RequestOptions::JSON => ['user_id' => $this->user_id, 'query' => $query, 'scope' => $search_scope === 'facts' ? 'edges' : 'nodes', 'limit' => $limit]]);
    ...
    {
        return array_map(fn(array $edge): array => ['fact' => $edge['fact'], 'created_at' => $edge['created_at']], $edges);
    }
    protected function mapNodes(array $nodes): array
    {
        return array_map(fn(array $node): array => ['name' => $node['name'], 'summary' => $node['summary']], $nodes);
    }
}
