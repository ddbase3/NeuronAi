...
class PGSQLSelectTool extends Tool
{
     ...
     */
    protected array $forbiddenPatterns = ['/^\s*(INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|REPLACE)\s+/i', '/\b(INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|REPLACE)\s+/i', '/;\s*(INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|REPLACE)\s+/i', '/\bINTO\s+OUTFILE\s+/i', '/\bLOAD\s+DATA\s+/i', '/\bSET\s+/i', '/\bCALL\s+/i', '/\bEXEC(UTE)?\s+/i'];
    ...
    {
        parent::__construct('pgsql_select_query', 'Use this tool only to run SELECT query against the PostgreSQL database.
This the tool to use only to gather information from the PostgreSQL database.');
    ...
    {
        return [new ToolProperty('query', PropertyType::STRING, 'The parameterized SELECT query with named placeholders (e.g., "SELECT name, email FROM users WHERE name = :name. Use named parameters (:parameter_name) for all dynamic values.', \true), new ArrayProperty('parameters', 'Key-value pairs for parameter binding where keys match the named placeholders in the query (without the colon). Example: {"name": "John Doe", "email": "%john%", "id": 123}. Leave empty if no parameters are needed.', \false, items: new ObjectProperty(name: 'parameter', properties: [new ToolProperty('name', PropertyType::STRING, 'Parameter name', \true), new ToolProperty('value', PropertyType::STRING, 'Parameter value', \true)]))];
    }
    /**
    ...
    protected function performAdditionalSecurityChecks(string $query): bool
    {
        ...
        $dangerousFunctions = ['pg_exec', 'pg_query', 'system', 'exec', 'shell_exec', 'passthru', 'eval'];
    ...
    protected function splitStatements(string $query): array
    {
        ...
        return array_filter(array_map(trim(...), explode(';', $query)), fn(string $stmt): bool => $stmt !== '');
    }
    /**
