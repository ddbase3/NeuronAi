...
class PGSQLSchemaTool extends Tool
{
    ...
    {
        parent::__construct('analyze_pgsql_database_schema', 'Retrieves PostgreSQL database schema information including tables, columns, relationships, and indexes.
...
and performance optimization. If you already know the database structure, you can skip this step.');
    ...
    protected function getTables(): array
    {
        ...
        foreach ($results as $row) {
            ...
            if (!isset($tables[$tableName])) {
                $tables[$tableName] = ['name' => $tableName, 'engine' => 'PostgreSQL', 'estimated_rows' => $this->getTableRowCount($tableName), 'comment' => $row['table_comment'], 'columns' => [], 'primary_key' => [], 'unique_keys' => [], 'indexes' => []];
            }
            if ($row['column_name']) {
                // Map PostgreSQL types to a more readable format
                $fullType = $this->formatPostgreSQLType($row);
                $column = ['name' => $row['column_name'], 'type' => $row['data_type'], 'full_type' => $fullType, 'nullable' => $row['is_nullable'] === 'YES', 'default' => $row['column_default'], 'auto_increment' => str_contains((string) $row['extra'], 'auto_increment'), 'comment' => $row['column_comment']];
    ...
    protected function getIndexes(): array
    {
        ...
        foreach ($results as $row) {
            ...
            $indexes[] = ['table' => $row['tablename'], 'name' => $row['indexname'], 'unique' => str_contains((string) $row['indexdef'], 'UNIQUE'), 'type' => $this->extractIndexType($row['indexdef']), 'columns' => $cleanColumns === [] ? $columns : $cleanColumns];
        }
        return $indexes;
    }
    ...
    protected function addCommonPatterns(string &$output, array $tables): void
    {
        ...
        foreach ($tables as $table) {
            foreach ($table['columns'] as $column) {
                if (in_array($column['type'], ['timestamp without time zone', 'timestamp with time zone', 'timestamptz', 'date', 'time']) && (str_contains(strtolower((string) $column['name']), 'created') || str_contains(strtolower((string) $column['name']), 'updated'))) {
        ...
        foreach ($tables as $table) {
            foreach ($table['columns'] as $column) {
                if (in_array($column['type'], ['character varying', 'varchar', 'text', 'character']) && (str_contains(strtolower((string) $column['name']), 'name') || str_contains(strtolower((string) $column['name']), 'title') || str_contains(strtolower((string) $column['name']), 'description'))) {
