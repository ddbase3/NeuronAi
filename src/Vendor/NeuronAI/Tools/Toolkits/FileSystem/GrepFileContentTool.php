...
class GrepFileContentTool extends Tool
{
    ...
    {
        parent::__construct(name: 'grep_file_content', description: 'Search for a regex pattern in a file.');
    }
    protected function properties(): array
    {
        return [ToolProperty::make(name: 'file_path', type: PropertyType::STRING, description: 'Path to the file to search.'), ToolProperty::make(name: 'pattern', type: PropertyType::STRING, description: 'Regular expression pattern to search for.')];
    }
    public function __invoke(string $file_path, string $pattern): string
    {
        ...
        foreach ($matches[0] as $index => $match) {
            ...
            for ($i = 0; $i < $lineCount; $i++) {
                $lineLength = mb_strlen($lines[$i]) + 1;
                // +1 for newline
