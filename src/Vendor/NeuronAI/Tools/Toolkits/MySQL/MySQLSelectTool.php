...
class MySQLSelectTool extends Tool
{
    ...
    protected array $forbiddenStatements = ['INSERT', 'UPDATE', 'DELETE', 'DROP', 'CREATE', 'ALTER', 'TRUNCATE', 'REPLACE', 'MERGE', 'CALL', 'EXECUTE', 'INTO', 'OUTFILE', 'DUMPFILE', 'LOAD_FILE'];
    public function __construct(protected PDO $pdo)
    {
        parent::__construct('mysql_select_query', 'Use this tool only to run SELECT query against the MySQL database to gather information.
...
Always use backticks around identifiers that are reserved keywords.');
    ...
    {
        return [new ToolProperty(name: 'query', type: PropertyType::STRING, description: 'The SELECT query. Use backticks (`) around table/column names that are MySQL reserved keywords (e.g., `character`, `order`, `group`, `index`, `key`, `value`, `date`, `time`). Use named placeholders (:parameter_name) for all dynamic values. Examples: "SELECT id, name FROM `character` WHERE type = :type", "SELECT * FROM `order` WHERE status = :status"', required: \true), new ArrayProperty(name: 'parameters', description: 'Key-value pairs for parameter binding where keys match the named placeholders in the query (without the colon). Example: {"name": "John Doe", "email": "%john%", "id": 123}. Ignore if no parameters are needed.', required: \false, items: new ObjectProperty(name: 'parameter', properties: [new ToolProperty('name', PropertyType::STRING, 'Parameter name', \true), new ToolProperty('value', PropertyType::STRING, 'Parameter value', \true)]))];
    }
    /**
