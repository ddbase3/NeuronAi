...
class SupadataVideoTranscriptTool extends Tool
{
    ...
    {
        parent::__construct('get_transcription', 'Retrieve the transcription of a youtube video.');
    }
    protected function properties(): array
    {
        return [new ToolProperty(name: 'video_url', type: PropertyType::STRING, description: 'The URL of the YouTube video you want to retrieve the transcription for.', required: \true)];
    }
    public function __invoke(string $video_url): string
    {
        $response = $this->getClient($this->key)->get('youtube/transcript?url=' . $video_url . '&text=true');
