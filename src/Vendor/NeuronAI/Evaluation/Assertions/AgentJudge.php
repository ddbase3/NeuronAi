...
use NeuronAi\Vendor\NeuronAI\Evaluation\JudgeScoreOutput;
...
class AgentJudge extends AbstractAssertion
{
     ...
     */
    public function __construct(protected AgentInterface $judge, protected string $criteria, protected float $threshold = 0.7, protected ?string $reference = null, protected array $examples = [])
    {
    }
    public function evaluate(mixed $actual): AssertionResult
    {
        ...
        $result = $this->judge->structured(new UserMessage($prompt), JudgeScoreOutput::class);
        $passed = $result->score >= $this->threshold;
        if ($passed) {
            return AssertionResult::pass($result->score, $result->reasoning, $this->buildContext());
        }
        return AssertionResult::fail($result->score, "Score {$result->score} below threshold {$this->threshold}. Reasoning: {$result->reasoning}", $this->buildContext());
    }
    /**
    ...
    {
        return ['threshold' => $this->threshold, 'criteria' => $this->criteria, 'reference' => $this->reference];
    }
}
