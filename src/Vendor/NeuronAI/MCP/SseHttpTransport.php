...
class SseHttpTransport implements McpTransportInterface
{
    ...
    {
        $this->httpClient = new Client(['timeout' => $config['timeout'] ?? 30, 'verify' => $config['verify'] ?? \true]);
    }
    /**
    ...
    public function connect(): void
    {
        ...
        try {
            ...
            $context = stream_context_create(['http' => ['method' => 'GET', 'header' => $headerString, 'timeout' => $this->config['timeout'] ?? 30], 'ssl' => ['verify_peer' => $this->config['verify'] ?? \true, 'verify_peer_name' => $this->config['verify'] ?? \true]]);
            ...
            if (isset($meta['wrapper_data']) && is_array($meta['wrapper_data'])) {
                foreach ($meta['wrapper_data'] as $header) {
                    // Check status code
                    if (stripos((string) $header, 'HTTP/') === 0 && in_array(preg_match('/HTTP\/\d\.\d\s+200/', (string) $header), [0, \false], \true)) {
                        $this->cleanup();
    ...
    {
        $timeout = microtime(\true) + 10;
        // 10 second timeout
        $endpointReceived = \false;
        while (!$endpointReceived && microtime(\true) < $timeout) {
            ...
            if (!$endpointReceived) {
                usleep(10000);
                // 10ms sleep to prevent busy waiting
            }
        }
        if (!$endpointReceived) {
    ...
    public function send(array $data): void
    {
        ...
        try {
            ...
            $response = $this->httpClient->post($this->postEndpointUrl, ['headers' => $headers, 'body' => $jsonData]);
    ...
    public function receive(): array
    {
        ...
        $timeout = microtime(\true) + ($this->config['timeout'] ?? 30);
        $received = \false;
        $response = null;
        while (!$received && microtime(\true) < $timeout) {
            ...
            if ($data === \false) {
                ...
                while (($pos = strpos($this->sseBuffer, "\n\n")) !== \false) {
                    ...
                    if ($parsed['event'] === 'message' && $parsed['data'] !== '') {
                        try {
                            $message = json_decode($parsed['data'], \true, 64, JSON_THROW_ON_ERROR);
    ...
    protected function resolveEndpointUrl(string $base, string $relative): string
    {
        ...
        if (str_contains($relative, '://') || str_starts_with($relative, '//')) {
            ...
            if (($relativeParts['scheme'] ?? 'https') !== ($baseParts['scheme'] ?? 'https') || ($relativeParts['host'] ?? '') !== ($baseParts['host'] ?? '') || ($relativeParts['port'] ?? null) !== ($baseParts['port'] ?? null)) {
