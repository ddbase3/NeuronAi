...
class McpClient
{
    ...
    public function __construct(array $config)
    {
        if (isset($config['transport']) && $config['transport'] instanceof McpTransportInterface) {
            ...
            $this->transport = $isAsync ? new SseHttpTransport($config) : new StreamableHttpTransport($config);
        } else {
    ...
    {
        $request = ['jsonrpc' => '2.0', 'id' => ++$this->requestId, 'method' => 'initialize', 'params' => ['protocolVersion' => '2024-11-05', 'capabilities' => (object) ['sampling' => new stdClass()], 'clientInfo' => (object) ['name' => 'neuron-ai', 'version' => '1.0.0']]];
    ...
    public function listTools(): array
    {
        $tools = [];
        do {
            $request = ['jsonrpc' => '2.0', 'id' => ++$this->requestId, 'method' => 'tools/list'];
    ...
    public function callTool(string $toolName, array $arguments = []): array
    {
        ...
        $request = ['jsonrpc' => '2.0', 'id' => ++$this->requestId, 'method' => 'tools/call', 'params' => ['name' => $toolName, ...$arguments !== [] ? ['arguments' => $arguments] : ['arguments' => new stdClass()]]];
        $this->transport->send($request);
        $response = $this->transport->receive();
        if ($response['id'] !== $this->requestId) {
            throw new McpException('Invalid response ID');
