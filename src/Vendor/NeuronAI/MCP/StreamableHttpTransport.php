...
use NeuronAi\Vendor\GuzzleHttp\Psr7\Request;
...
class StreamableHttpTransport implements McpTransportInterface
{
    ...
    {
        $this->httpClient = new Client(['timeout' => $config['timeout'] ?? 30, 'headers' => ['Accept' => 'application/json, text/event-stream', 'Content-Type' => 'application/json', 'User-Agent' => 'neuron-ai/1.0.0']]);
    }
    /**
    ...
    public function receive(): array
    {
        ...
        try {
            ...
            try {
                return json_decode($response, \true, 64, JSON_THROW_ON_ERROR);
            } catch (JsonException $e) {
                ...
                return json_decode($json, \true, 64, JSON_THROW_ON_ERROR);
            }
        } catch (JsonException $e) {
