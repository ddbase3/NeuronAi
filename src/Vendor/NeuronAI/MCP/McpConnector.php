...
class McpConnector
{
    ...
    {
        return ['config' => $this->config, 'only' => $this->only, 'exclude' => $this->exclude];
    }
    ...
    public function tools(): array
    {
        ...
        $tools = array_filter($this->client()->listTools(), fn(array $tool): bool => !in_array($tool['name'], $this->exclude) && ($this->only === [] || in_array($tool['name'], $this->only)));
    ...
    {
        $tool = Tool::make(name: $item['name'], description: $item['description'] ?? null, annotations: $item['annotations'] ?? [])->setCallable(new CallableMcpTool(connector: $this, item: $item));
    ...
    {
        return new ToolProperty(name: $name, type: $type, description: $prop['description'] ?? null, required: $required, enum: $prop['items']['enum'] ?? $prop['enum'] ?? []);
    }
    /**
     * @param array<string, mixed> $prop
     * @throws ArrayPropertyException
    ...
    {
        return new ArrayProperty(name: $name, description: $prop['description'] ?? null, required: $required, items: new ToolProperty(name: 'type', type: PropertyType::from($prop['items']['type'] ?? 'string')));
    }
    /**
     * @param array<string, mixed> $prop
     * @throws ArrayPropertyException
    ...
    {
        return new ObjectProperty(name: $name, description: $prop['description'] ?? null, required: $required);
    }
    /**
