...
namespace NeuronAi\Vendor\GuzzleHttp\Handler;

use NeuronAi\Vendor\GuzzleHttp\Exception\ConnectException;
use NeuronAi\Vendor\GuzzleHttp\Exception\RequestException;
use NeuronAi\Vendor\GuzzleHttp\Multiplexing;
use NeuronAi\Vendor\GuzzleHttp\Promise as P;
use NeuronAi\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use NeuronAi\Vendor\GuzzleHttp\Promise\PromiseInterface;
use NeuronAi\Vendor\GuzzleHttp\Psr7;
use NeuronAi\Vendor\GuzzleHttp\TransferStats;
use NeuronAi\Vendor\GuzzleHttp\TransportSharing;
use NeuronAi\Vendor\GuzzleHttp\Utils;
...
class StreamHandler
{
    ...
    public function __construct(array $options = [])
    {
        foreach ($options as $name => $_) {
            if (!isset(self::KNOWN_CONSTRUCTOR_OPTIONS[$name])) {
                \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.14', \sprintf('The "%s" StreamHandler constructor option is unknown; guzzlehttp/guzzle 8.0 will reject unknown constructor options.', (string) $name));
            }
        }
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options): void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    private static function triggerConflictingStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        foreach ($streamContext as $wrapper => $contextOptions) {
            ...
            foreach ($contextOptions as $option => $_) {
                ...
                \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing stream_context.%s.%s in the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $wrapper, $option, $conflictingOptions[$wrapper][$option]));
            }
        }
    }
    private static function triggerUnsupportedStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing PHP stream context options outside the built-in stream handler allow-list to the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject stream context options outside the allow-list. Deprecated option%s: %s.', \count($unsupportedOptions) === 1 ? '' : 's', \implode(', ', $unsupportedOptions)));
    }
    /**
     * @return string[]
     */
