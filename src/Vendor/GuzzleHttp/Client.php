...
use NeuronAi\Vendor\GuzzleHttp\Cookie\CookieJar;
use NeuronAi\Vendor\GuzzleHttp\Cookie\CookieJarInterface;
use NeuronAi\Vendor\GuzzleHttp\Exception\GuzzleException;
use NeuronAi\Vendor\GuzzleHttp\Exception\InvalidArgumentException;
use NeuronAi\Vendor\GuzzleHttp\Handler\CurlShareHandleState;
use NeuronAi\Vendor\GuzzleHttp\Handler\CurlVersion;
use NeuronAi\Vendor\GuzzleHttp\Promise as P;
use NeuronAi\Vendor\GuzzleHttp\Promise\PromiseInterface;
use NeuronAi\Vendor\Psr\Http\Message\RequestInterface;
use NeuronAi\Vendor\Psr\Http\Message\ResponseInterface;
use NeuronAi\Vendor\Psr\Http\Message\StreamInterface;
use NeuronAi\Vendor\Psr\Http\Message\UriInterface;
/**
 * @final
 */
class Client implements ClientInterface, \NeuronAi\Vendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    {
        \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.1', '%s::%s() is deprecated and will be removed in 8.0.', __CLASS__, __FUNCTION__);
        if (\count($args) < 1) {
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
    ...
    public function request(string $method, $uri = '', array $options = []): ResponseInterface
    {
        $normalizedMethod = Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = \true;
    ...
    private static function warnAboutRequestLevelHandler(array $options): void
    {
        ...
        \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $request = Psr7\Utils::modifyRequest($request, ['version' => '1.1']);
        } elseif (!self::isProtocolVersionValid($protocolVersion)) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
    ...
    private static function createBodyStream($body): StreamInterface
    {
        ...
        if (\is_scalar($body)) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option): array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
    ...
    {
        if ('' === $version) {
            \NeuronAi\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
